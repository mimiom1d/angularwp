...
if( ! class_exists('gcacf_field_color_picker') ) :

class gcacf_field_color_picker extends gcacf_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = __("Color Picker",'gcacf');
	...
	function input_admin_enqueue_scripts() {
		...
		if( !isset($wp_scripts->registered['iris']) ) {
		    ...
		    wp_localize_script('wp-color-picker', 'wpColorPickerL10n', array(
		        'clear'			=> __('Clear', 'gcacf' ),
		        'defaultString'	=> __('Default', 'gcacf' ),
		        'pick'			=> __('Select Color', 'gcacf' ),
		        'current'		=> __('Current Color', 'gcacf' )
		    )); 
	...
	*/
	...
	function render_field( $field ) {
		...
		// vars
		$text_input = gcacf_get_sub_array( $field, array('id', 'class', 'name', 'value') );
		$hidden_input = gcacf_get_sub_array( $field, array('name', 'value') );
		
		
		// html
		?>
		<div class="gcacf-color-picker">
...
// initialize
gcacf_register_field_type( 'gcacf_field_color_picker' );
