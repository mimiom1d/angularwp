...
/*
*  GCACF 3rd Party Compatibility Class
*
*  All the logic for 3rd party functionality
*
*  @class 		gcacf_third_party
*  @package		GCACF
*  @subpackage	Core
*/

if( ! class_exists('gcacf_third_party') ) :
...
class gcacf_third_party {
	...
	*/
	...
	function __construct() {
		...
		if( function_exists('espresso_version') ) {
			add_filter('gcacf/get_post_types', array($this, 'ee_get_post_types'), 10, 2);
		}
	...
	*  tabify_posttypes
	*
	*  This function removes GCACF post types from the tabify edit screen (post type selection sidebar)
	*
	*  @type	function
	*  @date	9/10/12
	*  @since	3.5.1
	...
	*/
	...
	function tabify_posttypes( $posttypes ) {
		...
		// unset
		unset( $posttypes['gcacf-field-group'] );
		unset( $posttypes['gcacf-field'] );
	...
	function tabify_add_meta_boxes( $post_type ) {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				// vars
				$id = "gcacf-{$field_group['key']}";
				$title = 'GCACF: ' . $field_group['title'];
	...
	*/
	...
	function doing_dark_mode() {
		wp_enqueue_style('gcacf-dark', gcacf_get_url('assets/css/gcacf-dark.css'), array(), GCACF_VERSION );
	}
	
}

new gcacf_third_party();
