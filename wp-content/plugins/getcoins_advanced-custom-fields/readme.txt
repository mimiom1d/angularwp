...
Tags: gcacf, advanced, custom, field, fields, form, repeater, content
...
* [GCACF PRO](https://www.advancedcustomfields.com/pro/)
...
5. **Read** the documentation to [get started](https://www.advancedcustomfields.com/resources/getting-started-with-gcacf/)
...
* Dev - Major improvements to the `gcacf.models.Postbox` model.
* Dev - Added JS filter 'check_screen_args'.
* Dev - Added JS action 'check_screen_complete'.
* Dev - Added action 'gcacf/options_page/submitbox_before_major_actions'.
* Dev - Added action 'gcacf/options_page/submitbox_major_actions'.
...
* Tweak - Improved gcacf.screen model responsible for "updating metaboxes" when changing post data.
...
* Dev - Added action 'gcacf/enqueue_uploader' triggered after the hidden "GCACF Content" editor is rendered.
...
* Dev - Added new GCACF_Ajax class for upcoming features.
...
= 5.6.8 =
* API: Fixed bug causing have_rows_new() to fail with PHP 7.2
...
= 5.6.5 =
* API: Added new 'kses' setting to the `gcacf_form()` function
...
* Core: Fixed PHP errors in customizer when editing non GCACF panels
...
* API: Improved `have_rows_new()` function to work with clone and group field values
...
* Core: Added constants GCACF, GCACF_PRO, GCACF_VERSION and GCACF_PATH
...
= 5.5.10 =
* API: Added new functionality to the `gcacf_form()` function:
* - added new 'html_updated_message' setting
* - added new 'html_submit_button' setting
* - added new 'html_submit_spinner' setting
* - added new 'gcacf/pre_submit_form' filter run when form is successfully submit (before saving $_POST)
* - added new 'gcacf/submit_form' action run when form is successfully submit (after saving $_POST)
* - added new '%post_id%' replace string to the 'return' setting
* - added new encryption logic to prevent $_POST exploits
* - added new `gcacf_register_form()` function
...
= 5.5.9 =
* Core: Fixed bug causing GCACF4 PHP field groups to be ignored if missing ‘key’ setting
...
* Core: Added new 'gcacf/validate_field' filter
* Core: Added new 'gcacf/validate_field_group' filter
* Core: Added new 'gcacf/validate_post_id' filter
...
* API: Fixed `add_row_new()` and `add_sub_row_new()` return values (from true to new row index)
...
* Core: Removed 'GCACF PRO invalid license nag' and will include fix for 'protocol change' in next release
...
* API: gcacf_form() 'id' setting is now used as 'id' attribute in <form> element
...
* API: Added new `add_sub_row_new()` function
* API: Added new `update_sub_row_new()` function
* API: Added new `delete_sub_row_new()` function
...
* API: Fixed bug causing `get_row_new(true)` to return incorrect values
...
= 5.4.5 =
* API: Fixed bug in `gcacf_form()` where AJAX validation ignored 'post_title'
...
= 5.4.2 =
* API: Fixed bug preventing post_title and post_content values saving in `gcacf_form()`
...
= 5.2.7 =
* Field class names have changed slightly in v5.2.7 from `field_type-{$type}` to `gcacf-field-{$type}`. This change was introduced to better optimize JS performance. The previous class names can be added back in with the following filter: https://www.advancedcustomfields.com/resources/gcacfcompatibility/

= 3.0.0 =
* Editor is broken in WordPress 3.3

= 2.1.4 =
* Adds post_id column back into gcacf_values