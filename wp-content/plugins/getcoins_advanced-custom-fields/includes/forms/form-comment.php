...
/*
*  GCACF Comment Form Class
*
*  All the logic for adding fields to comments
*
*  @class 		gcacf_form_comment
*  @package		GCACF
*  @subpackage	Forms
*/

if( ! class_exists('gcacf_form_comment') ) :
...
class gcacf_form_comment {
	...
	*  edit_comment
	*
	*  This function is run on the admin comment.php page and will render the GCACF fields within custom metaboxes to look native
	*
	*  @type	function
	*  @date	19/10/13
	*  @since	5.0.0
	...
	function edit_comment( $comment ) {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				// vars
				$o = array(
					'id'			=> 'gcacf-'.$field_group['ID'],
					'key'			=> $field_group['key'],
					//'style'			=> $field_group['style'],
					'label'			=> $field_group['label_placement'],
					'edit_url'		=> '',
					'edit_title'	=> __('Edit field group', 'gcacf'),
					//'visibility'	=> $visibility
				);
				
				
				// edit_url
				if( $field_group['ID'] && gcacf_current_user_can_admin() ) {
				...
				?>
				<div id="gcacf-<?php echo $field_group['ID']; ?>" class="stuffbox">
					<h3 class="hndle"><?php echo $field_group['title']; ?></h3>
					<div class="inside">
						<?php gcacf_render_fields( $fields, $post_id, 'div', $field_group['instruction_placement'] ); ?>
						<script type="text/javascript">
						if( typeof gcacf !== 'undefined' ) {
								
							gcacf.newPostbox(<?php echo json_encode($o); ?>);	
	...
	*/
	...
	function comment_form_field_comment( $html ) {
		...
		ob_start();
			...
			echo '<div class="gcacf-comment-fields gcacf-fields -clear">';
			
			foreach( $field_groups as $field_group ) {
				
				$fields = gcacf_get_fields( $field_group );
				
				gcacf_render_fields( $fields, $post_id, 'p', $field_group['instruction_placement'] );
	...
	*/
	...
	function save_comment( $comment_id ) {
		...
		if( !gcacf_verify_nonce('comment') ) {
			return $comment_id;
		}
		
		
		// kses
    	if( isset($_POST['gcacf']) ) {
	    	$_POST['gcacf'] = wp_kses_post_deep( $_POST['gcacf'] );
    	}
		
	    
	    // validate and save
	    if( gcacf_validate_save_post(true) ) {
			gcacf_save_post( "comment_{$comment_id}" );
		}
