...
if( ! class_exists('GCACF_Ajax_Query_Terms') ) :

class GCACF_Ajax_Query_Terms extends GCACF_Ajax_Query {
	
	/** @var string The AJAX action name */
	var $action = 'gcacf/ajax/query_terms';
	...
	*/
	...
	function get_results( $args ) {
		...
		// get terms
		$groups = gcacf_get_grouped_terms( $args );
		...
		foreach( $groups as $label => $terms ) {
			...
			if( $this->search && !isset($args['orderby']) ) {
				$terms = gcacf_order_by_search( $terms, $this->search );
			}
		...
		$taxonomies = gcacf_get_array($args['taxonomy']);
...
gcacf_new_instance('GCACF_AJAX_Query_Terms');
