...
if( ! class_exists('gcacf_form_customizer') ) :

class gcacf_form_customizer {
	...
	*/
	...
	function save_widget( $instance, $new_instance, $old_instance, $widget ) {
		...
		// append values
		$data['values'] = $new_instance['gcacf'];
		
		
		// append fields (name => key relationship) - used later in 'gcacf/get_field_reference' for customizer previews
		foreach( $data['values'] as $k => $v ) {
			
			// get field
			$field = gcacf_get_field( $k );
	...
	*  settings
	*
	*  This function will return an array of cutomizer settings that include GCACF data
	...
	*/
	...
	function settings( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// bail early if no gcacf
			if( !is_array($value) || !isset($value['gcacf']) ) continue;
			
			
			// set data	
			$setting->gcacf = $value['gcacf'];
	...
	*/
	...
	function customize_preview_init( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// append gcacf_value to preview_values
		...
		// add filters
		add_filter('gcacf/pre_load_value', array($this, 'pre_load_value'), 10, 3);
		add_filter('gcacf/pre_load_reference', array($this, 'pre_load_reference'), 10, 3);
	...
	*/
	...
	function customize_save( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// save gcacf data
			gcacf_save_post( $data['post_id'], $data['values'] );
			
			
			// remove [gcacf] data from saved widget array
	...
	*  pre_update_option
	*
	*  this function will remove the [gcacf] data from widget insance
	*
	*  @type	function
	*  @date	22/03/2016
	*  @since	5.3.2
	...
	*/
	...
	function pre_update_option( $value, $option, $old_value ) {
		...
		foreach( $value as $i => $widget ) {
			...
			// bail ealry if no gcacf
			if( !isset($widget['gcacf']) ) continue;
...
(function($) {
	...
	// because GCACF generates a unique ID on the WYSIWYG field, this string will not match causing the preview function to bail.
	...
	WidgetControl._getInputsSignature = function( inputs ) {
		...
		for( var i in signature ) {
			...
			// bail ealry if gcacf is found
			if( bit.indexOf('gcacf') !== -1 ) continue;
	...
	// either way, this function is not needed and will break GCACF fields 
