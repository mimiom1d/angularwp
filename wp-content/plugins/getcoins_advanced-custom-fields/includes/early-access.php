...
if( !class_exists('gcacf_early_access') ):

class gcacf_early_access {
	
	/** @var string The plugin basename */
	var $basename = 'advanced-custom-fields/gcacf.php';
	...
	*/
	...
	function __construct() {
		...
		if( !GCACF_EARLY_ACCESS ) return;
		
		
		// vars
		$this->access = (string) GCACF_EARLY_ACCESS;
		//$this->basename = apply_filters('gcacf/get_info', 'basename');
	...
	*/
	...
	function get_plugin_info() {
		...
		// var
		$transient_name = 'gcacf_early_access_info';
	...
	*/
	...
	function check_plugins_transient( $transient ) {
		...
		// bail early if gcacf was not checked
		// - rules out possible included file in theme / plugin
		if( !isset($transient->checked[ $basename ]) ) return $transient;
		
		
		// flush cache if no 'gcacf' update exists
		// flush cache if 'gcacf' update does not contain early access info
		// flush cache if 'gcacf' update contains different early access info
	...
	*/
	...
	function modify_plugin_update_message( $plugin_data, $response ) {
		...
		// display message
		echo ' <em>' . __('(Early access enabled)', 'gcacf') . '</em>';
