...
if( ! class_exists('GCACF_WPML_Compatibility') ) :

class GCACF_WPML_Compatibility {
	...
	*/
	function __construct() {
		...
		// update settings
		gcacf_update_setting('default_language', $sitepress->get_default_language());
		gcacf_update_setting('current_language', $sitepress->get_current_language());
		...
		add_action('gcacf/verify_ajax', array($this, 'verify_ajax'));
		
		// prevent 'gcacf-field' from being translated
		add_filter('get_translatable_documents', array($this, 'get_translatable_documents'));
		
		// check if 'gcacf-field-group' is translatable
		if( $this->is_translatable() ) {
			
			// actions
			add_action('gcacf/upgrade_500_field_group',		array($this, 'upgrade_500_field_group'), 10, 2);
			add_action('icl_make_duplicate',				array($this, 'icl_make_duplicate'), 10, 4);
			
			// filters
			add_filter('gcacf/settings/save_json',			array($this, 'settings_save_json'));
			add_filter('gcacf/settings/load_json',			array($this, 'settings_load_json'));
		}
	}
	...
	*  is_translatable
	*
	*  Returns true if the gcacf-field-group post type is translatable.
	*  Also adds compatibility with GCACF4 settings
	*
	*  @date	10/04/2015
	*  @since	5.2.3
	*
	...
	*/
	function is_translatable() {
		...
		if( !gcacf_is_array($post_types) ) {
			return false;
		}
		
		// prevent 'gcacf-field' from being translated
		if( !empty($post_types['gcacf-field']) ) {
			$post_types['gcacf-field'] = 0;
			$sitepress->set_setting('custom_posts_sync_option', $post_types);
		}
		
		// when upgrading to version 5, review 'gcacf' setting
		// update 'gcacf-field-group' if 'gcacf' is translatable, and 'gcacf-field-group' does not yet exist
		if( !empty($post_types['gcacf']) && !isset($post_types['gcacf-field-group']) ) {
			$post_types['gcacf-field-group'] = 1;
			$sitepress->set_setting('custom_posts_sync_option', $post_types);
		}
		
		// return true if gcacf-field-group is translatable
		if( !empty($post_types['gcacf-field-group']) ) {
			return true;
		}
	...
	*/
	function upgrade_500_field_group($field_group, $ofg) {
		...
		// get translation rows (old gcacf4 and new gcacf5)
		...
		$new_row = $wpdb->get_row($wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}icl_translations WHERE element_type=%s AND element_id=%d", 
			'post_gcacf-field-group', $field_group['ID']
		), ARRAY_A);
	...
	*/
	function settings_save_json( $path ) {	
		...
		// ammend
		$path = untrailingslashit($path) . '/' . gcacf_get_setting('current_language');
	...
	*/	
	function settings_load_json( $paths ) {
		...
		foreach( $paths as $i => $path ) {
			$paths[ $i ] = untrailingslashit($path) . '/' . gcacf_get_setting('current_language');
		}}
	...
	*/
	function icl_make_duplicate( $master_post_id, $lang, $postarr, $id ) {
		...
		if( $postarr['post_type'] != 'gcacf-field-group' ) {
			return;
		}
		
		// update the lang
		gcacf_update_setting('current_language', $lang);
		
		// duplicate field group specifying the $post_id
		gcacf_duplicate_field_group( $master_post_id, $id );
	...
	*
	*  Removes 'gcacf-field' from the available post types for translation.
	*
	*  @type	function
	*  @date	17/8/17
	*  @since	5.6.0
	...
	*/
	function get_translatable_documents( $icl_post_types ) {
		...
		// unset
		unset( $icl_post_types['gcacf-field'] );
...
gcacf_new_instance('GCACF_WPML_Compatibility');
