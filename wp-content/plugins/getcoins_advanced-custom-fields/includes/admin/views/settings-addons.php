<div class="wrap gcacf-settings-wrap">
	
	<h1><?php _e("Add-ons",'gcacf'); ?></h1>
		...
		<?php if( !empty($json) ): ?>
			...
			<?php foreach( $json as $addon ): 
				...
				$addon = wp_parse_args($addon, array(
			        ...
			        "thumbnail"		=> "",
			        "url"			=> "",
			        "btn"			=> __("Download & Install",'gcacf'),
			        "btn_color"		=> ""
				));
				
				?>
				
				<div class="gcacf-box add-on add-on-<?php echo $addon['slug']; ?>">
					...
					<div class="footer">
						<?php if( apply_filters("gcacf/is_add_on_active/slug={$addon['slug']}", false ) ): ?>
							<a class="button" disabled="disabled"><?php _e("Installed",'gcacf'); ?></a>
						<?php else: ?>
