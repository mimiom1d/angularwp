...
if( ! class_exists('gcacf_field_taxonomy') ) :

class gcacf_field_taxonomy extends gcacf_field {
	...
	*/
	...
	function initialize() {
		...
		// ajax
		add_action('wp_ajax_gcacf/fields/taxonomy/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_gcacf/fields/taxonomy/query',	array($this, 'ajax_query'));
		add_action('wp_ajax_gcacf/fields/taxonomy/add_term',		array($this, 'ajax_add_term'));
		
		
		// actions
		add_action('gcacf/save_post', array($this, 'save_post'), 15, 1);
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		// filters
		$args = apply_filters('gcacf/fields/taxonomy/query', $args, $field, $options['post_id']);
		$args = apply_filters('gcacf/fields/taxonomy/query/name=' . $field['name'], $args, $field, $options['post_id'] );
		$args = apply_filters('gcacf/fields/taxonomy/query/key=' . $field['key'], $args, $field, $options['post_id'] );
		
		
		// get terms
		$terms = gcacf_get_terms( $args );
		...
		if( $is_hierarchical ) {
			...
			// update vars
			$limit = gcacf_maybe_get( $args, 'number', $limit );
			$offset = gcacf_maybe_get( $args, 'offset', $offset );
			
			
			// get parent
			$parent = gcacf_maybe_get( $args, 'parent', 0 );
			$parent = gcacf_maybe_get( $args, 'child_of', $parent );
	...
	*/
	...
	function get_term_title( $term, $field, $post_id = 0 ) {
		...
		// filters
		$title = apply_filters('gcacf/fields/taxonomy/result', $title, $term, $field, $post_id);
		$title = apply_filters('gcacf/fields/taxonomy/result/name=' . $field['_name'] , $title, $term, $field, $post_id);
		$title = apply_filters('gcacf/fields/taxonomy/result/key=' . $field['key'], $title, $term, $field, $post_id);
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {
		...
		// get valid terms
		$value = gcacf_get_valid_terms($value, $field['taxonomy']);
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		if( $field['save_terms'] ) {
			...
			$term_ids = gcacf_get_array( $value );
...
<div <?php gcacf_esc_attr_e($div); ?>>
	...
	<div class="gcacf-actions -hover">
		<a href="#" class="gcacf-icon -plus gcacf-js-tooltip small" data-name="add" title="<?php echo esc_attr($taxonomy->labels->add_new_item); ?>"></a>
	</div>
	<?php endif;
		...
		if( !empty($field['value']) ) {
			...
			if( !empty($terms) ) {
				...
				foreach( array_keys($terms) as $i ) {
					...
					// vars
					$term = gcacf_extract_var( $terms, $i );
	...
	*/
	...
	function render_field_checkbox( $field ) {
		...
		// include walker
		gcacf_include('includes/walkers/class-gcacf-walker-taxonomy-field.php');
		
		
		// vars
		$args = array(
			'taxonomy'     		=> $field['taxonomy'],
			'show_option_none'	=> sprintf( _x('No %s', 'No terms', 'gcacf'), strtolower($taxonomy_obj->labels->name) ),
			'hide_empty'   		=> false,
			'style'        		=> 'none',
			'walker'       		=> new GCACF_Taxonomy_Field_Walker( $field ),
		);
		
		
		// filter for 3rd party customization
		$args = apply_filters('gcacf/fields/taxonomy/wp_list_categories', $args, $field);
		$args = apply_filters('gcacf/fields/taxonomy/wp_list_categories/name=' . $field['_name'], $args, $field);
		$args = apply_filters('gcacf/fields/taxonomy/wp_list_categories/key=' . $field['key'], $args, $field);
		
		?>
<div class="categorychecklist-holder">
	<ul class="gcacf-checkbox-list gcacf-bl">
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Taxonomy','gcacf'),
			'instructions'	=> __('Select the taxonomy to be displayed','gcacf'),
			'type'			=> 'select',
			'name'			=> 'taxonomy',
			'choices'		=> gcacf_get_taxonomy_labels(),
		));
		
		
		// field_type
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Appearance','gcacf'),
			'instructions'	=> __('Select the appearance of this field','gcacf'),
			'type'			=> 'select',
			'name'			=> 'field_type',
			'optgroup'		=> true,
			'choices'		=> array(
				__("Multiple Values",'gcacf') => array(
					'checkbox' => __('Checkbox', 'gcacf'),
					'multi_select' => __('Multi Select', 'gcacf')
				),
				__("Single Value",'gcacf') => array(
					'radio' => __('Radio Buttons', 'gcacf'),
					'select' => _x('Select', 'noun', 'gcacf')
				)
			)
		));
		...
		// add_term
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Create Terms','gcacf'),
			'instructions'	=> __('Allow new terms to be created whilst editing','gcacf'),
			'name'			=> 'add_term',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// save_terms
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Save Terms','gcacf'),
			'instructions'	=> __('Connect selected terms to the post','gcacf'),
			'name'			=> 'save_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// load_terms
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Load Terms','gcacf'),
			'instructions'	=> __('Load value from posts terms','gcacf'),
			'name'			=> 'load_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
	...
	*/
	...
	function ajax_add_term() {
		...
		// verify nonce
		if( !gcacf_verify_ajax() ) {
			die();
		}		
		
		// load field
		$field = gcacf_get_field( $args['field_key'] );
		if( !$field ) {
			die();
		}
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			wp_send_json_error(array(
				'error'	=> sprintf( __('User unable to add new %s', 'gcacf'), $taxonomy_label )
			));
		}
		...
		if( $args['term_name'] ) {
			...
			if( term_exists($args['term_name'], $field['taxonomy'], $args['term_parent']) ) {
				wp_send_json_error(array(
					'error'	=> sprintf( __('%s already exists', 'gcacf'), $taxonomy_label )
				));
			}
			...
			wp_send_json_success(array(
				'message'		=> sprintf( __('%s added', 'gcacf'), $taxonomy_label ),
		...
		?><p class="gcacf-submit">
			<button class="gcacf-submit-button button button-primary" type="submit"><?php _e("Add", 'gcacf'); ?></button>
		</p>
		</form><?php
...
// initialize
gcacf_register_field_type( 'gcacf_field_taxonomy' );
