...
/*
*  GCACF Taxonomy Form Class
*
*  All the logic for adding fields to taxonomy terms
*
*  @class 		gcacf_form_taxonomy
*  @package		GCACF
*  @subpackage	Forms
*/

if( ! class_exists('gcacf_form_taxonomy') ) :
...
class gcacf_form_taxonomy {
	...
	*/
	...
	function add_term( $taxonomy ) {
		...
		// vars
		$post_id = gcacf_get_term_post_id( $taxonomy, 0 );
		...
		if( !empty($field_groups) ) {
			...
			// wrap
			echo '<div id="gcacf-term-fields" class="gcacf-fields -clear">';
			
			// loop
			foreach( $field_groups as $field_group ) {
				$fields = gcacf_get_fields( $field_group );
				gcacf_render_fields( $fields, $post_id, 'div', 'field' );
			}
	...
	*/
	...
	function edit_term( $term, $taxonomy ) {
		...
		// vars
		$post_id = gcacf_get_term_post_id( $term->taxonomy, $term->term_id );
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				echo '<table class="form-table">';
					$fields = gcacf_get_fields( $field_group );
					gcacf_render_fields( $fields, $post_id, 'tr', 'field' );
				echo '</table>';
...
if( $this->view == 'add' ): ?>
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		gcacf.doAction('remove', $fields);
		
		// reset HTML
		$fields.html( html );
		
		// action for 3rd party customization
		gcacf.doAction('append', $fields);
...
<?php
	...
	*/
	...
	function save_term( $term_id, $tt_id, $taxonomy ) {
		...
		// vars
		$post_id = gcacf_get_term_post_id( $taxonomy, $term_id );
		
		
		// verify and remove nonce
		if( !gcacf_verify_nonce('taxonomy') ) return $term_id;
	...
	*/
	...
	function delete_term( $term, $tt_id, $taxonomy, $deleted_term ) {
		...
		if( gcacf_isset_termmeta() ) return $term;
