...
if( ! class_exists('GCACF_Updates') ) :

class GCACF_Updates {
	
	/** @var string The GCACF_Updates version */
	var $version = '2.4';
	...
	*  request
	*
	*  Makes a request to the GCACF connect server.
	*
	*  @date	8/4/17
	*  @since	5.5.10
	*
	...
	*/
	...
	function request( $query = 'index.php', $body = null ) {
		...
		if( $this->dev ) {
			...
			gcacf_log('gcacf connect: '. $url, $body);
		}
	...
	*/
	...
	function get_plugin_info( $id = '', $force_check = false ) {
		...
		// var
		$transient_name = 'gcacf_plugin_info_' . $id;
	...
	*/
	...
	function get_plugin_updates( $force_check = false ) {
		...
		// var
		$transient_name = 'gcacf_plugin_updates';
		...
		$post = array(
			...
			'gcacf'			=> wp_json_encode(array(
				'gcacf_version'	=> get_option('gcacf_version'),
				'gcacf_pro'		=> (defined('GCACF_PRO') && GCACF_PRO),
			)),
		);
	...
	*/
	...
	function refresh_plugins_transient() {
		...
		delete_transient('gcacf_plugin_updates');
	}
	
	/*
	*  modify_plugins_transient
	*
	*  Called when WP updates the 'update_plugins' site transient. Used to inject GCACF plugin update info.
	*
	*  @date	16/01/2014
	*  @since	5.0.0
	*
...
*  gcacf_updates
*
*  The main function responsible for returning the one true gcacf_updates instance to functions everywhere.
*  Use this function like you would a global variable, except without needing to declare the global.
*
*  Example: <?php $gcacf_updates = gcacf_updates(); ?>
*
*  @date	9/4/17
*  @since	5.5.12
*
...
function gcacf_updates() {
	global $gcacf_updates;
	if( !isset($gcacf_updates) ) {
		$gcacf_updates = new GCACF_Updates();
	}
	return $gcacf_updates;
}


/*
*  gcacf_register_plugin_update
*
*  Alias of gcacf_updates()->add_plugin().
*
*  @type	function
*  @date	12/4/17
*  @since	5.5.10
...
function gcacf_register_plugin_update( $plugin ) {
	gcacf_updates()->add_plugin( $plugin );	
}
