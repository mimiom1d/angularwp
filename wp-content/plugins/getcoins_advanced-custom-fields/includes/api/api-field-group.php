...
function gcacf_is_field_group_key( $key = '' ) {
	...
	if( gcacf_is_local_field_group($key) ) return true;
...
function gcacf_get_valid_field_group( $field_group = false ) {
	...
	// filter
	$field_group = apply_filters('gcacf/validate_field_group', $field_group);

	
	// translate
	$field_group = gcacf_translate_field_group( $field_group );
...
function gcacf_translate_field_group( $field_group ) {
	...
	if( $l10n && $l10n_textdomain ) {
		...
		// translate
		$field_group['title'] = gcacf_translate( $field_group['title'] );
		
		
		// filters
		$field_group = apply_filters( "gcacf/translate_field_group", $field_group );
...
function gcacf_get_field_groups( $args = false ) {
	...
	// query DB for child ids
		...
		$posts = get_posts(array(
			'post_type'					=> 'gcacf-field-group',
			'posts_per_page'			=> -1,
			'orderby' 					=> 'menu_order title',
			'order' 					=> 'asc',
			'suppress_filters'			=> false, // allow WPML to modify the query
			'post_status'				=> array('publish', 'gcacf-disabled'),
	...
	foreach( $post_ids as $post_id ) {
		...
		$field_groups[] = gcacf_get_field_group( $post_id );
	...
	$field_groups = apply_filters('gcacf/get_field_groups', $field_groups);
	
	
	// filter via args
	if( $args ) {
		
		$field_groups = gcacf_filter_field_groups( $field_groups, $args );
...
/*
*  gcacf_filter_field_groups
*
*  This function is used by gcacf_get_field_groups to filter out fields groups based on location rules
*
*  @type	function
*  @date	29/11/2013
*  @since	5.0.0
...
function gcacf_filter_field_groups( $field_groups, $args = false ) {
	...
	foreach( $keys as $key ) {
		...
		// get visibility
		$visibility = gcacf_get_field_group_visibility( $field_groups[ $key ], $args );
...
/*
*  gcacf_get_field_group_visibility
*
...
function gcacf_get_field_group_visibility( $field_group, $args = array() ) {
	...
	// get screen
	$screen = gcacf_get_location_screen( $args, $field_group );
	...
	foreach( $field_group['location'] as $group_id => $group ) {
		...
		foreach( $group as $rule_id => $rule ) {
			...
			if( !gcacf_match_location_rule( $rule, $screen ) ) {
...
function gcacf_get_field_group( $selector = null ) {
	...
	if( $type == 'ID' ) {
		...
		$field_group = _gcacf_get_field_group_by_id( $selector );
	
	// key	
	} else {
		
		$field_group = _gcacf_get_field_group_by_key( $selector );
	...
	// validate
	$field_group = gcacf_get_valid_field_group( $field_group );
	
	
	// filter for 3rd party customization
	$field_group = apply_filters('gcacf/get_field_group', $field_group);
	
	
	// update cache
	// - Use key instead of ID for best compatibility (not all field groups exist in the DB)
	$cache_key = gcacf_set_cache("get_field_group/key={$field_group['key']}", $field_group);
	
	
	// update cache reference
	// - allow cache to return if using an ID selector
	gcacf_set_cache_reference("get_field_group/ID={$field_group['ID']}", $cache_key);
...
function _gcacf_get_field_group_by_id( $post_id = 0 ) {
	...
	if( empty($post) || $post->post_type != 'gcacf-field-group' ) return false;
	...
	// override with JSON
	if( gcacf_is_local_field_group( $field_group['key'] ) ) {
		
		// load JSON field
		$local = gcacf_get_local_field_group( $field_group['key'] );
...
function _gcacf_get_field_group_by_key( $key = '' ) {
	
	// try JSON before DB to save query time
	if( gcacf_is_local_field_group( $key ) ) {
		
		return gcacf_get_local_field_group( $key );
		
	}
	
	
	// vars
	$post_id = gcacf_get_field_group_id( $key );
	...
	// return
	return _gcacf_get_field_group_by_id( $post_id );
	
}


/*
*  gcacf_get_field_group_id
*
...
function gcacf_get_field_group_id( $key = '' ) {
	...
	$args = array(
		...
		'post_status'		=> array('publish', 'gcacf-disabled', 'trash'),
		'gcacf_group_key'		=> $key
	);
...
function gcacf_update_field_group( $field_group = array() ) {
	...
	$field_group = gcacf_parse_types( $field_group );
    ...
    $post_status = $extract['active'] ? 'publish' : 'gcacf-disabled';
	...
	add_filter( 'wp_unique_post_slug', 'gcacf_update_field_group_wp_unique_post_slug', 100, 6 ); 
	...
	do_action('gcacf/update_field_group', $field_group);
	
	
	// clear cache
	gcacf_delete_cache("get_field_group/key={$field_group['key']}");
...
function gcacf_update_field_group_wp_unique_post_slug( $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug ) {
		
	if( $post_type == 'gcacf-field-group' ) {
...
function gcacf_duplicate_field_group( $selector = 0, $new_post_id = 0 ) {
	...
	$field_group = gcacf_get_field_group( $selector );
	...
	if( !$new_post_id ) {
		...
		$field_group['title'] .= ' (' . __("copy", 'gcacf') . ')';
		
	}
	
	
	// save
	$field_group = gcacf_update_field_group( $field_group );
	
	
	// get fields
	$fields = gcacf_get_fields( $orig_field_group );
	
	
	// duplicate fields
	gcacf_duplicate_fields( $fields, $field_group['ID'] );
	
	
	// action for 3rd party customization
	do_action('gcacf/duplicate_field_group', $field_group);
...
function gcacf_get_field_count( $field_group ) {
	...
	if( !$field_group['ID'] ) {
		...
		$count = gcacf_count_local_fields( $field_group['key'] );	
	...
	// filter for 3rd party customization
	$count = apply_filters('gcacf/get_field_count', $count, $field_group);
...
function gcacf_delete_field_group( $selector = 0 ) {
	...
	// action for 3rd party customization
	do_action('gcacf/delete_field_group', $field_group);
...
function gcacf_trash_field_group( $selector = 0 ) {
	...
	// action for 3rd party customization
	do_action('gcacf/trash_field_group', $field_group);
...
function gcacf_untrash_field_group( $selector = 0 ) {
	...
	// action for 3rd party customization
	do_action('gcacf/untrash_field_group', $field_group);
...
function gcacf_get_field_group_style( $field_group ) {
	...
	// return	
	return apply_filters('gcacf/get_field_group_style', $style, $field_group);
}


/*
*  gcacf_import_field_group
*
...
function gcacf_import_field_group( $field_group ) {
	...
	// extract fields
	$fields = gcacf_extract_var($field_group, 'fields');
	
	
	// format fields
	$fields = gcacf_prepare_fields_for_import( $fields );
	...
	if( $field_group['ID'] ) {
		...
		// load fields
		$db_fields = gcacf_get_fields_by_id( $field_group['ID'] );
		$db_fields = gcacf_prepare_fields_for_import( $db_fields );
...
/*
*  gcacf_prepare_field_group_for_export
*
*  description
*
*  @type	function
...
function gcacf_prepare_field_group_for_export( $field_group ) {
	
	// extract some args
	$extract = gcacf_extract_vars($field_group, array(
		'ID',
	...
	// prepare fields
	$field_group['fields'] = gcacf_prepare_fields_for_export( $field_group['fields'] );
	
	
	// filter for 3rd party customization
	$field_group = apply_filters('gcacf/prepare_field_group_for_export', $field_group);
...
/**
*  gcacf_get_field_group_edit_link
*
...
*  @return	string
*/
function gcacf_get_field_group_edit_link( $post_id ) {
	if( $post_id && gcacf_current_user_can_admin() ) {
