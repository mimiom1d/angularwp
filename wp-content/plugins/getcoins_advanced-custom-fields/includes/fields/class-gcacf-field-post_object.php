...
if( ! class_exists('gcacf_field_post_object') ) :

class gcacf_field_post_object extends gcacf_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = __("Post Object",'gcacf');
		...
		// extra
		add_action('wp_ajax_gcacf/fields/post_object/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_gcacf/fields/post_object/query',	array($this, 'ajax_query'));
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		// filters
		$args = apply_filters('gcacf/fields/post_object/query', $args, $field, $options['post_id']);
		$args = apply_filters('gcacf/fields/post_object/query/name=' . $field['name'], $args, $field, $options['post_id'] );
		$args = apply_filters('gcacf/fields/post_object/query/key=' . $field['key'], $args, $field, $options['post_id'] );
		...
		// optgroup or single
		$post_type = gcacf_get_array( $args['post_type'] );
	...
	*/
	...
	function get_post_title( $post, $field, $post_id = 0, $is_search = 0 ) {
		...
		// filters
		$title = apply_filters('gcacf/fields/post_object/result', $title, $post, $field, $post_id);
		$title = apply_filters('gcacf/fields/post_object/result/name=' . $field['_name'], $title, $post, $field, $post_id);
		$title = apply_filters('gcacf/fields/post_object/result/key=' . $field['key'], $title, $post, $field, $post_id);
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		if( !$field['multiple'] && gcacf_is_array($value) ) {
...
// initialize
gcacf_register_field_type( 'gcacf_field_post_object' );
