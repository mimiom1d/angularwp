...
if( ! class_exists('GCACF_Form_User') ) :

class GCACF_Form_User {
	...
	*/
	...
	function admin_enqueue_scripts() {
		...
		if( !gcacf_is_screen(array('profile', 'user', 'user-edit')) ) {
			return;
		}
	...
	*  render
	*
	*  This function will render GCACF fields for a given $post_id parameter
	*
	*  @type	function
	*  @date	7/10/13
	*  @since	5.0.0
	...
	*/
	...
	function render( $args = array() ) {
		...
		if( $args['el'] == 'div') {
			$before = '<div class="gcacf-user-' . $args['view'] . '-fields gcacf-fields -clear">';
			$after = '</div>';
		}
		...
		foreach( $field_groups as $field_group ) {
			...
			// render
			echo $before;
			gcacf_render_fields( $fields, $post_id, $args['el'], $field_group['instruction_placement'] );
			echo $after;
		}
				
		// actions
		add_action('gcacf/input/admin_footer', array($this, 'admin_footer'), 10, 1);
	}
...
<?php
	...
	*/
	...
	function save_user( $user_id ) {
		...
		// verify nonce
		if( !gcacf_verify_nonce('user') ) {
			return $user_id;
		}
		
	    // save
	    if( gcacf_validate_save_post(true) ) {
			gcacf_save_post( "user_$user_id" );
		}
	}		
}

// instantiate
gcacf_new_instance('GCACF_Form_User');
