...
function get_field_new( $selector, $post_id = false, $format_value = true ) {
	...
	// get field
	$field = gcacf_maybe_get_field( $selector, $post_id );
	...
	// get value for field
	$value = gcacf_get_value( $post_id, $field );
	...
	if( $format_value ) {
		...
		// get value for field
		$value = gcacf_format_value( $value, $post_id, $field );
...
*  the_field_new()
*
*  This function is the same as echo get_field_new().
*
*  @type	function
*  @since	1.0.3
*  @date	29/01/13
...
function the_field_new( $selector, $post_id = false, $format_value = true ) {
	
	$value = get_field_new($selector, $post_id, $format_value);
...
function get_field_object_new( $selector, $post_id = false, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$field['value'] = gcacf_format_value( $field['value'], $post_id, $field );
...
function get_fields_new( $post_id = false, $format_value = true ) {
	
	// vars
	$fields = get_field_objects_new( $post_id, $format_value );
	$meta = array();
...
function get_field_objects_new( $post_id = false, $format_value = true, $load_value = true ) {
	...
	// filter post_id
	$post_id = gcacf_get_valid_post_id( $post_id );
	$info = gcacf_get_post_id_info( $post_id );
	...
	foreach( $meta as $k => $v ) {
		...
		$field = gcacf_maybe_get_field( $field_key );
...
function have_rows_new( $selector, $post_id = false ) {
	...
	$active_loop = gcacf_get_loop('active');
	$previous_loop = gcacf_get_loop('previous');
	...
	// loop has changed
		...
		$sub_field = gcacf_get_sub_field($selector, $active_loop['field']);
	...
	if( $new_parent_loop || $new_child_loop ) {
		...
		if( $new_parent_loop ) {
			...
			$field = get_field_object_new( $selector, $post_id, false );
			$value = gcacf_extract_var( $field, 'value' );
			$name = $field['name'];
		...
		if( !gcacf_is_array($value) ) return false;
		
		
		// allow for non repeatable data (group)
		if( gcacf_get_field_type_prop($field['type'], 'have_rows') === 'single' ) {
			$value = array( $value );
		}
		
		
		// add loop
		$active_loop = gcacf_add_loop(array(
			'selector'	=> $selector,
...
function the_row_new( $format = false ) {
	
	// vars
	$i = gcacf_get_loop('active', 'i');
	...
	// update
	gcacf_update_loop('active', 'i', $i);
	
	
	// return
	return get_row_new( $format );
	
}

function get_row_new( $format = false ) {
	
	// vars
	$loop = gcacf_get_loop('active');
	...
	// get value
	$value = gcacf_maybe_get( $loop['value'], $loop['i'] );
	
	
	// bail early if no current value
	// possible if get_row_layout_new() is called before the_row_new()
	...
	if( $format ) {
		...
		if( gcacf_get_field_type_prop($field['type'], 'have_rows') === 'single' ) {
			...
			// format value
			$value = gcacf_format_value( $value, $loop['post_id'], $field );
		...
		// multiple rows
			...
			$value = gcacf_format_value( $loop['value'], $loop['post_id'], $field );
			$value = gcacf_maybe_get( $value, $loop['i'] );
...
function the_row_index_new() {
	
	echo get_row_index_new();
...
function get_row_sub_field_new( $selector ) {
	
	// vars
	$row = gcacf_get_loop('active');
	...
	// attempt to find sub field
	$sub_field = gcacf_get_sub_field($selector, $row['field']);
...
function get_row_sub_value_new( $selector ) {
...
function has_sub_field_new( $field_name, $post_id = false ) {
	
	// vars
	$r = have_rows_new( $field_name, $post_id );
...
function has_sub_fields_new( $field_name, $post_id = false ) {
	
	return has_sub_field_new( $field_name, $post_id );
...
function get_sub_field_new( $selector = '', $format_value = true ) {
	
	// get sub field
	$sub_field = get_sub_field_object_new( $selector, $format_value );
...
function the_sub_field_new( $field_name, $format_value = true ) {
	
	$value = get_sub_field_new( $field_name, $format_value );
...
function get_sub_field_object_new( $selector, $format_value = true, $load_value = true ) {
	...
	// attempt to find sub field
	$sub_field = get_row_sub_field_new($selector);
	...
	if( $load_value ) {
		...
		$sub_field['value'] = get_row_sub_value_new( $sub_field['key'] );
	...
	if( $format_value ) {
		...
		// get value for field
		$sub_field['value'] = gcacf_format_value( $sub_field['value'], $row['post_id'], $sub_field );
...
function get_row_layout_new() {
	...
	// return
	if( isset($row['gcacf_fc_layout']) ) {
		
		return $row['gcacf_fc_layout'];
...
*  gcacf_shortcode()
*
*  This function is used to add basic shortcode support for the GCACF plugin
*  eg. [gcacf field="heading" post_id="123" format_value="1"]
*
*  @type	function
*  @since	1.1.1
*  @date	29/01/13
...
function gcacf_shortcode( $atts ) {
	...
	$value = get_field_new( $field, $post_id, $format_value );
...
add_shortcode('gcacf', 'gcacf_shortcode');
...
function update_field_new( $selector, $value, $post_id = false ) {
	...
	// get field
	$field = gcacf_maybe_get_field( $selector, $post_id, false );
	...
	// save
	return gcacf_update_value( $value, $post_id, $field );
...
function update_sub_field_new( $selector, $value, $post_id = false ) {
	...
	if( is_array($selector) ) {
		...
		$post_id = gcacf_get_valid_post_id( $post_id );
		$sub_field = gcacf_maybe_get_sub_field( $selector, $post_id, false );
		
	} else {
		
		$post_id = gcacf_get_loop('active', 'post_id');
		$sub_field = get_row_sub_field_new( $selector );
	...
	// update
	return gcacf_update_value( $value, $post_id, $sub_field );
...
function delete_field_new( $selector, $post_id = false ) {
	...
	// delete
	return gcacf_delete_value( $post_id, $field );
...
function delete_sub_field_new( $selector, $post_id = false ) {
	
	return update_sub_field_new( $selector, null, $post_id );
...
function add_row_new( $selector, $row = false, $post_id = false ) {
	...
	// update value
	gcacf_update_value( $value, $post_id, $field );
...
function add_sub_row_new( $selector, $row = false, $post_id = false ) {
	...
	// get raw value
	$value = gcacf_get_value( $post_id, $sub_field );
	...
	// update
	gcacf_update_value( $value, $post_id, $sub_field );
...
function update_row_new( $selector, $i = 1, $row = false, $post_id = false ) {
...
function update_sub_row_new( $selector, $i = 1, $row = false, $post_id = false ) {
...
function delete_row_new( $selector, $i = 1, $post_id = false ) {
...
function delete_sub_row_new( $selector, $i = 1, $post_id = false ) {
...
function create_field_new( $field ) {

	gcacf_render_field( $field );
	
}

function render_field_new( $field ) {

	gcacf_render_field( $field );
	
}

function reset_the_repeater_field_new() {
	
	return reset_rows_new();
	
}

function the_repeater_field_new( $field_name, $post_id = false ) {
	
	return has_sub_field_new( $field_name, $post_id );
	
}

function the_flexible_field_new( $field_name, $post_id = false ) {
	
	return has_sub_field_new( $field_name, $post_id );
	
}

function gcacf_filter_post_id( $post_id ) {
	
	return gcacf_get_valid_post_id( $post_id );
