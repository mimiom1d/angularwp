...
class gcacf_local {
	...
	*/
	...
	function __construct() {
		...
		// actions
		add_action('gcacf/include_fields', 	array($this, 'gcacf_include_fields'), 5, 0);
		
		
		// filters
		add_filter('gcacf/get_field_groups',	array($this, 'gcacf_get_field_groups'), 20, 1);
	...
	*  is_enabled
	*
	*  This function will return true if gcacf_local functionality is enabled
	*
	*  @type	function
	*  @date	14/07/2016
	*  @since	5.4.0
	...
	*/
	...
	function is_enabled() {
		...
		if( !gcacf_get_setting('local') ) return false;
		
		
		// return
		return gcacf_is_filter_enabled('local');
	...
	*  is_ready
	*
	*  This function will return true when GCACF has included all field types and is ready to import
	...
	*/
	...
	function is_ready() {
		...
		return did_action('gcacf/include_fields');
		
	}
	
	
	/*
	*  gcacf_include_fields
	*
	...
	*/
	...
	function gcacf_include_fields() {
		...
		if( !empty($this->temp_fields) ) {
			...
			// prepare
			$this->temp_fields = gcacf_prepare_fields_for_import( $this->temp_fields );
	...
	*/
	...
	function get_field_groups() {
		...
		foreach( array_keys($this->groups) as $field_group_key ) {
			...
			$field_groups[] = gcacf_get_field_group( $field_group_key );
	...
	*/
	...
	function get_fields( $parent_key = '' ) {
		...
		foreach( $this->parents[ $parent_key ] as $field_key ) {
			...
			$fields[] = gcacf_get_field( $field_key );
	...
	*
	*  This function will override and add field groups to the `gcacf_get_field_groups()` results
	*
	*  @type	filter (gcacf/get_field_groups)
	*  @date	5/12/2013
	*  @since	5.0.0
	*
	...
	*/
	...
	function gcacf_get_field_groups( $field_groups ) {
...
*  Helpers
*
*  alias of gcacf()->local->functions
*
*  @type	function
*  @date	11/06/2014
*  @since	5.0.0
...
function gcacf_local() {
	
	return gcacf()->local;
	
}

function gcacf_disable_local() {
	
	gcacf_disable_filter('local');
	
}

function gcacf_enable_local() {
	
	gcacf_enable_filter('local');
	
}

function gcacf_reset_local() {
	
	return gcacf_local()->reset();
	
}


// field group
function gcacf_add_local_field_group( $field_group ) {
	
	return gcacf_local()->maybe_add_field_group( $field_group );
	
}

function gcacf_remove_local_field_group( $key = '' ) {
	
	return false;
	
}

function gcacf_is_local_field_group( $key = '' ) {
	
	return gcacf_local()->is_field_group( $key );
	
}

function gcacf_get_local_field_group( $key = '' ) {
	
	return gcacf_local()->get_field_group( $key );
	
}


// field groups
function gcacf_count_local_field_groups() {
	
	return gcacf_local()->count_field_groups();
	
}

function gcacf_have_local_field_groups() {
	
	return gcacf_local()->have_field_groups();
	
}

function gcacf_get_local_field_groups() {
	
	return gcacf_local()->get_field_groups();
	
}


// field
function gcacf_add_local_field( $field ) {
	
	return gcacf_local()->maybe_add_field( $field );
	
}

function gcacf_remove_local_field( $key = '' ) {
	
	return gcacf_local()->remove_field( $key );
	
}

function gcacf_is_local_field( $key = '' ) {
	
	return gcacf_local()->is_field( $key );
	
}

function gcacf_is_local_field_key( $key = '' ) {
	
	return gcacf_local()->is_field_key( $key );
	
}

function gcacf_is_local_field_name( $name = '' ) {
	
	return gcacf_local()->is_field_name( $name );
	
}

function gcacf_get_local_field( $key = '' ) {
	
	return gcacf_local()->get_field( $key );
	
}


// fields
function gcacf_count_local_fields( $key = '' ) {
	
	return gcacf_local()->count_fields( $key );
	
}

function gcacf_have_local_fields( $key = '' ) {
	
	return gcacf_local()->have_fields( $key );
	
}

function gcacf_get_local_fields( $key = '' ) {
	
	return gcacf_local()->get_fields( $key );
	
}

function gcacf_remove_local_fields( $key = '' ) {
	
	return gcacf_local()->remove_fields( $key );
	
}


// deprecated
function register_field_group_new( $field_group ) {
	
	gcacf_add_local_field_group( $field_group );
