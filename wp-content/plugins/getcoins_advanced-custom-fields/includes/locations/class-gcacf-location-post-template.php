...
if( ! class_exists('gcacf_location_post_template') ) :

class gcacf_location_post_template extends gcacf_location {
	...
	*/
	...
	function initialize() {
		...
		$this->label = __("Post Template",'gcacf');
		$this->category = 'post';
		$this->public = gcacf_version_compare('wp', '>=', '4.7');
	...
	*/
	...
	function rule_match( $result, $rule, $screen ) {
		...
		$templates = array();
		$post_id = gcacf_maybe_get( $screen, 'post_id' );
		$page_template = gcacf_maybe_get( $screen, 'page_template' );
		...
		if( gcacf_version_compare('wp', '>=', '4.7') ) {
			...
			$templates = gcacf_get_post_templates();
	...
	*/
	...
	function rule_values( $choices, $rule ) {
		...
		// vars
		$choices = array(
			'default' => apply_filters( 'default_page_template_title',  __('Default Template', 'gcacf') )
		);
...
// initialize
gcacf_register_location_rule( 'gcacf_location_post_template' );
