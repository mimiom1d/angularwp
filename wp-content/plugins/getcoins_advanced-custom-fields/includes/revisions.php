...
if( ! class_exists('gcacf_revisions') ) :

class gcacf_revisions {
	...
	*/
	...
	function __construct() {
		...
		add_filter('gcacf/validate_post_id', array($this, 'gcacf_validate_post_id'), 10, 2 );
	...
	*/
	...
	function wp_preview_post_fields( $fields ) {
		...
		if( gcacf_maybe_get_POST('wp-preview') !== 'dopreview' ) return $fields;
		
		
		// add to fields if GCACF has changed
		if( gcacf_maybe_get_POST('_gcacf_changed') ) {
			
			$fields['_gcacf_changed'] = 'different than 1';
	...
	*/
	...
	function wp_save_post_revision_check_for_changes( $return, $last_revision, $post ) {
		...
		// if gcacf has changed, return false and prevent WP from performing 'compare' logic
		if( gcacf_maybe_get_POST('_gcacf_changed') ) return false;
	...
	*
	*  This filter will add the GCACF fields to the returned array
	...
	*/
	...
	function wp_post_revision_fields( $fields, $post = null ) {
		...
		// validate page
		if( gcacf_is_screen('revision') || gcacf_is_ajax('get-revision-diffs') ) {
			
			// bail early if is restoring
			if( gcacf_maybe_get_GET('action') === 'restore' ) return $fields;
		...
		$append = array();
		$order = array();
		$post_id = gcacf_maybe_get($post, 'ID');
		...
		foreach( $meta as $name => $value ) {
			...
			$ancestors = gcacf_get_field_ancestors( $field );
			...
			if( !empty($ancestors) ) {
				...
				$oldest = gcacf_get_field( $ancestors[$count-1] );
		...
		if( !empty($append) ) {
			...
			// add prefix
			$append = gcacf_add_array_key_prefix($append, $prefix);
			$order = gcacf_add_array_key_prefix($order, $prefix);
			...
			// remove prefix
			$append = gcacf_remove_array_key_prefix($append, $prefix);
	...
	*/
	...
	function wp_post_revision_field( $value, $field_name, $post = null, $direction = false) {
		...
		// load field
		$field = gcacf_maybe_get_field( $field_name, $post_id );
	...
	*/
	...
	function wp_restore_post_revision( $post_id, $revision_id ) {
		...
		gcacf_copy_postmeta( $revision_id, $post_id );
		
		
		// Make sure the latest revision is also updated to match the new $post data
		// get latest revision
		$revision = gcacf_get_post_latest_revision( $post_id );
		...
		if( $revision ) {
			...
			gcacf_copy_postmeta( $revision_id, $revision->ID );
...
// initialize
gcacf()->revisions = new gcacf_revisions();

endif; // class_exists check


/*
*  gcacf_save_post_revision
*
...
function gcacf_save_post_revision( $post_id = 0 ) {
	...
	if( $revision ) {
		...
		gcacf_copy_postmeta( $post_id, $revision->ID );
...
/*
*  gcacf_get_post_latest_revision
*
...
function gcacf_get_post_latest_revision( $post_id ) {
