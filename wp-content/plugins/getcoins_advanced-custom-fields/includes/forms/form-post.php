...
if( ! class_exists('GCACF_Form_Post') ) :

class GCACF_Form_Post {
	...
	*/
	function initialize() {
		...
		$restricted = array('gcacf-field-group', 'attachment');
	...
	*  add_meta_boxes
	*
	*  Adds GCACF metaboxes for the given $post_type and $post.
	*
	*  @date	19/9/18
	*  @since	5.7.6
	*
	...
	*/
	function add_meta_boxes( $post_type, $post ) {
		...
		foreach( $field_groups as $field_group ) {
			...
			// vars
			$id = "gcacf-{$field_group['key']}";			// gcacf-group_123
			$title = $field_group['title'];				// Group 1
			$context = $field_group['position'];		// normal, side, gcacf_after_title
			...
			*/
			$priority = apply_filters('gcacf/input/meta_box_priority', $priority, $field_group);
			
			// set the visibility for this field group
			$visible = gcacf_get_field_group_visibility($field_group, array(
				'post_id'	=> $post->ID, 
				'post_type'	=> $post_type
			));
			...
			$this->postboxes[ $id ] = array(
				...
				'visible'	=> $visible,
				'edit'		=> gcacf_get_field_group_edit_link( $field_group['ID'] )
			);
			...
			if( $visible ) {
				...
				if( $count == 1 ) {
					$this->style = gcacf_get_field_group_style( $field_group );
				}
			}
		}}
	...
	*/
	function edit_form_after_title() {
		...
		// render 'gcacf_after_title' metaboxes
		do_meta_boxes( get_current_screen(), 'gcacf_after_title', $post );
			
		// clean up $wp_meta_boxes
		unset( $wp_meta_boxes['post']['gcacf_after_title'] );
		
		// render dynamic field group style
		echo '<style type="text/css" id="gcacf-style">' . $this->style . '</style>';
	}
	...
	*  render_meta_box
	*
	*  Renders the GCACF metabox HTML.
	*
	*  @date	19/9/18
	*  @since	5.7.6
	*
	...
	*/
	function render_meta_box( $post, $metabox ) {
		...
		if( $postbox['visible'] ) {
			...
			gcacf_render_fields( $fields, $post->ID, 'div', $field_group['instruction_placement'] );
		}
		...
		if( typeof gcacf !== 'undefined' ) {
			gcacf.newPostbox(<?php echo wp_json_encode($postbox); ?>);
		}	
		</script>
		<?php
	}
	...
	*
	*  Allows WP to insert a new post without title or post_content if GCACF data exists.
	*
	*  @date	16/07/2014
	*  @since	5.0.1
	*
	...
	*/
	function wp_insert_post_empty_content( $maybe_empty, $postarr ) {
		...
		// return false and allow insert if '_gcacf_changed' exists
		if( $maybe_empty && gcacf_maybe_get_POST('_gcacf_changed') ) {
			return false;
		}
	...
	*  Used to avoid triggering "gcacf/save_post" on dynamically created posts during save.
	*
	*  @type	function
	*  @date	26/06/2016
	*  @since	5.3.8
	...
	*/
	function allow_save_post( $post ) {
		...
		// restrict post types
		$restrict = array( 'auto-draft', 'revision', 'gcacf-field', 'gcacf-field-group' );
		if( in_array($post->post_type, $restrict) ) {
			$allow = false;
		}
		
		// disallow if the $_POST ID value does not match the $post->ID
		$form_post_id = (int) gcacf_maybe_get_POST('post_ID');
		...
		if( $post->post_type == 'revision' ) {
			...
			if( gcacf_maybe_get_POST('wp-preview') == 'dopreview' && $form_post_id === $post->post_parent) {
				$allow = true;
			}
		}
	...
	*/
	...
	function save_post( $post_id, $post ) {
		...
		if( post_type_supports($post->post_type, 'revisions') ) {
			gcacf_save_post_revision( $post_id );
		}
...
gcacf_new_instance('GCACF_Form_Post');
