...
if( ! class_exists('gcacf_field_page_link') ) :

class gcacf_field_page_link extends gcacf_field {
	...
	*/
	...
	function initialize() {
		...
		// extra
		add_action('wp_ajax_gcacf/fields/page_link/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_gcacf/fields/page_link/query',		array($this, 'ajax_query'));
	...
	*/
	...
	function ajax_query() {
		...
		// defaults
   		$options = gcacf_parse_args($_POST, array(
			'post_id'		=> 0,
			's'				=> '',
			'field_key'		=> '',
			'paged'			=> 1
		...
		if( !empty($field['taxonomy']) ) {
			...
			// decode terms
			$taxonomies = gcacf_decode_taxonomy_terms( $field['taxonomy'] );
		...
		// filters
		$args = apply_filters('gcacf/fields/page_link/query', $args, $field, $options['post_id']);
		$args = apply_filters('gcacf/fields/page_link/query/name=' . $field['name'], $args, $field, $options['post_id'] );
		$args = apply_filters('gcacf/fields/page_link/query/key=' . $field['key'], $args, $field, $options['post_id'] );
	...
	*/
	...
	function get_post_title( $post, $field, $post_id = 0, $is_search = 0 ) {
		...
		// filters
		$title = apply_filters('gcacf/fields/page_link/result', $title, $post, $field, $post_id);
		$title = apply_filters('gcacf/fields/page_link/result/name=' . $field['_name'], $title, $post, $field, $post_id);
		$title = apply_filters('gcacf/fields/page_link/result/key=' . $field['key'], $title, $post, $field, $post_id);
		...
		if( !empty($field['value']) ) {
			...
			if( !empty($posts) ) {
				...
				foreach( array_keys($posts) as $i ) {
					...
					// vars
					$post = gcacf_extract_var( $posts, $i );
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// allow_archives
		gcacf_render_field_setting( $field, array(
			'label'			=> __('Allow Archives URLs','gcacf'),
			'instructions'	=> '',
			'name'			=> 'allow_archives',
			'type'			=> 'true_false',
			'ui'			=> 1,
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		foreach( array_keys($value) as $i ) {
			...
			// vars
			$post = gcacf_extract_var( $value, $i );
...
// initialize
gcacf_register_field_type( 'gcacf_field_page_link' );
