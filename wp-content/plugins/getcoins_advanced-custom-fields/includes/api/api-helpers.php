...
function gcacf_has_setting( $name = '' ) {
	return gcacf()->has_setting( $name );
}
...
function gcacf_raw_setting( $name = '' ) {
	return gcacf()->get_setting( $name );
}


/*
*  gcacf_update_setting
*
*  alias of gcacf()->update_setting()
*
*  @type	function
*  @date	28/09/13
*  @since	5.0.0
...
function gcacf_update_setting( $name, $value ) {
	
	// validate name
	$name = gcacf_validate_setting( $name );
	
	// update
	return gcacf()->update_setting( $name, $value );
}
...
function gcacf_validate_setting( $name = '' ) {
	return apply_filters( "gcacf/validate_setting", $name );
}
...
function gcacf_get_setting( $name, $value = null ) {
	
	// validate name
	$name = gcacf_validate_setting( $name );
	
	// check settings
	if( gcacf_has_setting($name) ) {
		$value = gcacf_raw_setting( $name );
	}
	
	// filter
	$value = apply_filters( "gcacf/settings/{$name}", $value );
...
/*
*  gcacf_append_setting
*
*  This function will add a value into the settings array found in the gcacf object
*
*  @type	function
*  @date	28/09/13
*  @since	5.0.0
...
function gcacf_append_setting( $name, $value ) {
	
	// vars
	$setting = gcacf_raw_setting( $name );
	...
	// update
	return gcacf_update_setting( $name, $setting );
}
...
function gcacf_get_data( $name ) {
	return gcacf()->get_data( $name );
}
...
function gcacf_set_data( $name, $value ) {
	return gcacf()->set_data( $name, $value );
}
...
function gcacf_new_instance( $class ) {
	return gcacf()->new_instance( $class );
}
...
function gcacf_get_instance( $class ) {
	return gcacf()->get_instance( $class );
}
...
function gcacf_get_compatibility( $name ) {
	
	return apply_filters( "gcacf/compatibility/{$name}", false );
...
function gcacf_has_done( $name ) {
	
	// return true if already done
	if( gcacf_raw_setting("has_done_{$name}") ) {
		return true;
	}
	
	// update setting and return
	gcacf_update_setting("has_done_{$name}", true);
	return false;
}
...
*  gcacf_get_path
*
*  This function will return the path to a file within the GCACF plugin folder
*
*  @type	function
*  @date	28/09/13
*  @since	5.0.0
*
*  @param	$path (string) the relative path from the root of the GCACF plugin folder
*  @return	(string)
*/

function gcacf_get_path( $path = '' ) {
	
	return GCACF_PATH . $path;
...
*  gcacf_get_url
*
*  This function will return the url to a file within the GCACF plugin folder
*
*  @date	12/12/17
*  @since	5.6.8
*
*  @param	string $path The relative path from the root of the GCACF plugin folder
*  @return	string
*/

function gcacf_get_url( $path = '' ) {
	
	// define GCACF_URL to optimise performance
	if( !defined('GCACF_URL') ) {
		define( 'GCACF_URL', gcacf_get_setting('url') );
	}
	
	// return
	return GCACF_URL . $path;
...
*  gcacf_get_dir
*
*  Deprecated in 5.6.8. Use gcacf_get_url() instead.
*
*  @date	28/09/13
*  @since	5.0.0
*
...
function gcacf_get_dir( $path = '' ) {
	return gcacf_get_url( $path );
}
...
function gcacf_include( $file ) {
	
	$path = gcacf_get_path( $file );
...
function gcacf_include_once( $file = '' ) {
	$path = gcacf_get_path( $file );
	if( file_exists($path) ) {
		include_once( $path );
	}
...
function gcacf_get_external_path( $file, $path = '' ) {
...
function gcacf_get_external_dir( $file, $path = '' ) {
    
    return gcacf_plugin_dir_url( $file ) . $path;
	
}


/**
*  gcacf_plugin_dir_url
*
...
function gcacf_plugin_dir_url( $file ) {
...
function gcacf_parse_args( $args, $defaults = array() ) {
...
function gcacf_parse_types( $array ) {
	
	// return
	return array_map( 'gcacf_parse_type', $array );
...
function gcacf_get_view( $path = '', $args = array() ) {
	
	// allow view file name shortcut
	if( substr($path, -4) !== '.php' ) {
		
		$path = gcacf_get_path("includes/admin/views/{$path}.php");
...
function gcacf_merge_atts( $atts, $extra = array() ) {
...
function gcacf_nonce_input( $nonce = '' ) {
	
	echo '<input type="hidden" name="_gcacf_nonce" value="' . wp_create_nonce( $nonce ) . '" />';
...
function gcacf_extract_var( &$array, $key, $default = null ) {
...
function gcacf_extract_vars( &$array, $keys ) {
	
	$r = array();
	
	foreach( $keys as $key ) {
		
		$r[ $key ] = gcacf_extract_var( $array, $key );
...
function gcacf_get_sub_array( $array, $keys ) {
...
function gcacf_get_post_types( $args = array() ) {
	
	// vars
	$post_types = array();
	
	// extract special arg
	$exclude = gcacf_extract_var( $args, 'exclude', array() );
	$exclude[] = 'gcacf-field';
	$exclude[] = 'gcacf-field-group';
	...
	// filter
	$post_types = apply_filters('gcacf/get_post_types', $post_types, $args);
	
	// return
	return $post_types;
}

function gcacf_get_pretty_post_types( $post_types = array() ) {
	
	// get post types
	if( empty($post_types) ) {
		
		// get all custom post types
		$post_types = gcacf_get_post_types();
	...
	foreach( $post_types as $post_type ) {
		...
		// vars
		$label = gcacf_get_post_type_label($post_type);
...
function gcacf_get_post_type_label( $post_type ) {
...
*  gcacf_verify_nonce
*
*  This function will look at the $_POST['_gcacf_nonce'] value and return true or false
*
*  @type	function
*  @date	15/10/13
*  @since	5.0.0
...
function gcacf_verify_nonce( $value) {
	
	// vars
	$nonce = gcacf_maybe_get_POST('_gcacf_nonce');
...
function gcacf_verify_ajax() {
	
	// vars
	$nonce = isset($_REQUEST['nonce']) ? $_REQUEST['nonce'] : '';
	
	// bail early if not gcacf nonce
	if( !$nonce || !wp_verify_nonce($nonce, 'gcacf_nonce') ) {
		return false;
	}
	
	// action for 3rd party customization
	do_action('gcacf/verify_ajax');
...
function gcacf_get_image_sizes() {
	
	// vars
	$sizes = array(
		'thumbnail'	=>	__("Thumbnail",'gcacf'),
		'medium'	=>	__("Medium",'gcacf'),
		'large'		=>	__("Large",'gcacf')
	);
	...
	// add full end
	$sizes['full'] = __("Full Size",'gcacf');
	
	
	// filter for 3rd party customization
	$sizes = apply_filters( 'gcacf/get_image_sizes', $sizes );
...
function gcacf_get_image_size( $s = '' ) {
...
function gcacf_version_compare( $left = 'wp', $compare = '>', $right = '1' ) {
	...
	// remove '-beta1' or '-RC1'
	$left = gcacf_get_full_version($left);
	$right = gcacf_get_full_version($right);
...
function gcacf_get_full_version( $version = '1' ) {
...
function gcacf_get_terms( $args ) {
	...
	if( gcacf_version_compare('wp', '<', '4.5') ) {
...
function gcacf_get_taxonomy_terms( $taxonomies = array() ) {
	
	// force array
	$taxonomies = gcacf_get_array( $taxonomies );
	
	
	// get pretty taxonomy names
	$taxonomies = gcacf_get_pretty_taxonomies( $taxonomies );
	...
	foreach( array_keys($taxonomies) as $taxonomy ) {
		...
		foreach( $terms as $term ) {
			...
			$r[ $label ][ $k ] = gcacf_get_term_title( $term );
...
function gcacf_decode_taxonomy_terms( $strings = false ) {
	...
	foreach( $strings as $string ) {
		...
		// vars
		$data = gcacf_decode_taxonomy_term( $string );
...
function gcacf_decode_taxonomy_term( $value ) {
	...
	// allow for term_id (Used by GCACF v4)
	if( is_numeric($data['term']) ) {
		...
		// attempt get term via ID (GCACF4 uses ID)
...
function gcacf_get_array( $var = false, $delimiter = '' ) {
...
function gcacf_get_numeric( $value = '' ) {
...
function gcacf_get_posts( $args = array() ) {
	...
	if( $args['post__in'] ) {
		...
		$args['post__in'] = gcacf_get_array( $args['post__in'] );
		...
		//add_filter('query', '_gcacf_query_remove_post_type');
	...
	//remove_filter('query', '_gcacf_query_remove_post_type');
...
function _gcacf_query_remove_post_type( $sql ) {
...
function gcacf_get_grouped_posts( $args ) {
	...
	$post_types = gcacf_get_array( $args['post_type'] );
	$post_types_labels = gcacf_get_pretty_post_types($post_types);
	...
	if( !$is_single_post_type ) {
		add_filter('posts_orderby', '_gcacf_orderby_post_type', 10, 2);
	}
	...
	// remove this filter (only once)
	if( !$is_single_post_type ) {
		remove_filter('posts_orderby', '_gcacf_orderby_post_type', 10, 2);
	}
	...
	foreach( $post_types as $post_type ) {
		...
		if( is_post_type_hierarchical($post_type) && empty($args['s'])) {
			...
			$parent_id = gcacf_maybe_get($args, 'post_parent', 0);
			$offset = 0;
...
function _gcacf_orderby_post_type( $ordeby, $wp_query ) {
...
function gcacf_get_post_title( $post = 0, $is_search = false ) {
...
/*
		...
		if( $is_search && !empty($ancestors) ) {
			...
			// append
			$append .= ' | ' . __('Parent', 'gcacf') . ': ' . implode(' / ', $ancestors);
...
function gcacf_order_by_search( $array, $search ) {
	...
	foreach( array_keys($array) as $k ) {
		...
		$array[ '_' . $k ] = gcacf_extract_var( $array, $k );
	...
	foreach( array_keys($array) as $k ) {
		...
		$array[ substr($k,1) ] = gcacf_extract_var( $array, $k );
...
function gcacf_get_pretty_user_roles( $allowed = false ) {
	
	// vars
	$editable_roles = get_editable_roles();
	$allowed = gcacf_get_array($allowed);
	$roles = array();
...
function gcacf_get_grouped_users( $args = array() ) {
	...
	$min = 0;
	$max = 0;
	$users_per_page = gcacf_extract_var($args, 'users_per_page');
	$paged = gcacf_extract_var($args, 'paged');
	...
	// find array of post_type
	$user_roles = gcacf_get_pretty_user_roles($args['role']);
	...
	if( is_array($args['role']) ) {
		...
		// vars
		$roles = gcacf_extract_var($args, 'role');
	...
	foreach( $user_roles as $user_role_name => $user_role_label ) {
		...
		foreach( array_keys($users) as $key ) {
			...
			// extract user
			$user = gcacf_extract_var( $users, $key );
...
function gcacf_json_encode( $json ) {
...
function gcacf_str_exists( $needle, $haystack ) {
...
function gcacf_debug_start() {
	
	gcacf_update_setting( 'debug_start', memory_get_usage());
	
}

function gcacf_debug_end() {
	
	$start = gcacf_get_setting( 'debug_start' );
...
function gcacf_encode_choices( $array = array(), $show_keys = true ) {
...
function gcacf_decode_choices( $string = '', $array_keys = false ) {
...
function gcacf_str_replace( $string = '', $search_replace = array() ) {
...
gcacf_update_setting('php_to_js_date_formats', array(
...
gcacf_update_setting('php_to_js_time_formats', array(
...
function gcacf_split_date_time( $date_time = '' ) {
	
	// vars
	$php_date = gcacf_get_setting('php_to_js_date_formats');
	$php_time = gcacf_get_setting('php_to_js_time_formats');
...
function gcacf_convert_date_to_php( $date = '' ) {
	
	// vars
	$php_to_js = gcacf_get_setting('php_to_js_date_formats');
	$js_to_php = array_flip($php_to_js);
		
	
	// return
	return gcacf_str_replace( $date, $js_to_php );
	
}

/*
*  gcacf_convert_date_to_js
*
...
function gcacf_convert_date_to_js( $date = '' ) {
	...
	// return
	return gcacf_str_replace( $date, $php_to_js );
	
}


/*
*  gcacf_convert_time_to_php
*
...
function gcacf_convert_time_to_php( $time = '' ) {
	
	// vars
	$php_to_js = gcacf_get_setting('php_to_js_time_formats');
	$js_to_php = array_flip($php_to_js);
		
	
	// return
	return gcacf_str_replace( $time, $js_to_php );
	
}


/*
*  gcacf_convert_time_to_js
*
*  This fucntion converts a date format string from PHP to JS
*
*  @type	function
...
function gcacf_convert_time_to_js( $time = '' ) {
	...
	// return
	return gcacf_str_replace( $time, $php_to_js );
	
}


/*
*  gcacf_update_user_setting
*
*  description
*
*  @type	function
...
function gcacf_update_user_setting( $name, $value ) {
	...
	// get user settings
	$settings = get_user_meta( $user_id, 'gcacf_user_settings', true );
	
	
	// ensure array
	$settings = gcacf_get_array($settings);
	
	
	// delete setting (allow 0 to save)
	if( gcacf_is_empty($value) ) {
	...
	// update user data
	return update_metadata('user', $user_id, 'gcacf_user_settings', $settings);
	
}


/*
*  gcacf_get_user_setting
*
*  description
*
*  @type	function
...
function gcacf_get_user_setting( $name = '', $default = false ) {
...
function gcacf_in_array( $value = '', $array = false ) {
...
function gcacf_get_valid_post_id( $post_id = 0 ) {
	...
	if( is_object($post_id) ) {
		...
		// term
			...
			$post_id = gcacf_get_term_post_id( $post_id->taxonomy, $post_id->term_id );
	...
	if( $post_id == 'options' ) {
		...
		$dl = gcacf_get_setting('default_language');
		$cl = gcacf_get_setting('current_language');
	...
	$post_id = apply_filters('gcacf/validate_post_id', $post_id, $_post_id);
...
function gcacf_get_post_id_info( $post_id = 0 ) {
	...
	//if( gcacf_isset_cache($cache_key) ) return gcacf_get_cache($cache_key);
	...
	// string
		...
		// check if is taxonomy (GCACF < 5.5)
		// - avoid scenario where taxonomy exists with name of meta type
		if( !in_array($type, $meta) && gcacf_isset_termmeta($type) ) $type = 'term';
	...
	// update cache
	//gcacf_set_cache($cache_key, $info);
	
	
	// filter
	$info = apply_filters("gcacf/get_post_id_info", $info, $post_id);
...
gcacf_log( gcacf_get_post_id_info(4) );

gcacf_log( gcacf_get_post_id_info('post_4') );

gcacf_log( gcacf_get_post_id_info('user_123') );

gcacf_log( gcacf_get_post_id_info('term_567') );

gcacf_log( gcacf_get_post_id_info('category_204') );

gcacf_log( gcacf_get_post_id_info('comment_6') );

gcacf_log( gcacf_get_post_id_info('options_lol!') );

gcacf_log( gcacf_get_post_id_info('option') );

gcacf_log( gcacf_get_post_id_info('options') );

*/


/*
*  gcacf_isset_termmeta
*
...
function gcacf_isset_termmeta( $taxonomy = '' ) {
...
function gcacf_get_term_post_id( $taxonomy, $term_id ) {
	
	// WP < 4.4
	if( !gcacf_isset_termmeta() ) {
...
function gcacf_upload_files( $ancestors = array() ) {
	...
	// assign global _gcacfuploader for media validation
	$_POST['_gcacfuploader'] = end($ancestors);
	
	
	// file found!
	$attachment_id = gcacf_upload_file( $file );
	
	
	// update $_POST
	array_unshift($ancestors, 'gcacf');
	gcacf_update_nested_array( $_POST, $ancestors, $attachment_id );
...
function gcacf_upload_file( $uploaded_file ) {
...
function gcacf_update_nested_array( &$array, $ancestors, $value ) {
	...
	if( isset($array[ $k ]) ) {
		...
		return gcacf_update_nested_array( $array[ $k ], $ancestors, $value );
...
function gcacf_maybe_get( $array = array(), $key = 0, $default = null ) {
	
	return isset( $array[$key] ) ? $array[$key] : $default;
		
}

function gcacf_maybe_get_POST( $key = '', $default = null ) {
	
	return isset( $_POST[$key] ) ? $_POST[$key] : $default;
	
}

function gcacf_maybe_get_GET( $key = '', $default = null ) {
...
function gcacf_get_attachment( $attachment ) {
	...
	// video
		...
		// dimentions
		$response['width'] = gcacf_maybe_get($meta, 'width', 0);
		$response['height'] = gcacf_maybe_get($meta, 'height', 0);
...
function gcacf_get_truncated( $text, $length = 64 ) {
...
function gcacf_get_current_url() {
	...
	// test
	//$home = 'http://gcacf5/dev/wp-admin';
	//$url = $home . '/dev/wp-admin/api-template/gcacf_form';
...
/*
*  gcacf_current_user_can_admin
*
*  This function will return true if the current user can administrate the GCACF field groups
*
*  @type	function
*  @date	9/02/2015
*  @since	5.1.5
...
function gcacf_current_user_can_admin() {
	
	if( gcacf_get_setting('show_admin') && current_user_can(gcacf_get_setting('capability')) ) {
...
function gcacf_get_filesize( $size = 1 ) {
...
function gcacf_format_filesize( $size = 1 ) {
	
	// convert
	$bytes = gcacf_get_filesize( $size );
...
function gcacf_get_valid_terms( $terms = false, $taxonomy = 'category' ) {
	
	// force into array
	$terms = gcacf_get_array($terms);
...
/*
function gcacf_esc_html_deep( $value ) {
	
	// array
	if( is_array($value) ) {
		
		$value = array_map('gcacf_esc_html_deep', $value);
...
function gcacf_validate_attachment( $attachment, $field, $context = 'prepare' ) {
	...
	if( $context == 'upload' ) {
		...
		if( strpos($attachment['type'], 'image') !== false ) {
			...
			$file['width'] = gcacf_maybe_get($size, 0);
			$file['height'] = gcacf_maybe_get($size, 1);
	...
	// prepare
		...
		$file['size'] = gcacf_maybe_get($attachment, 'filesizeInBytes', 0);
		$file['width'] = gcacf_maybe_get($attachment, 'width', 0);
		$file['height'] = gcacf_maybe_get($attachment, 'height', 0);
	...
	if( $file['width'] || $file['height'] ) {
		...
		// width
		$min_width = (int) gcacf_maybe_get($field, 'min_width', 0);
		$max_width = (int) gcacf_maybe_get($field, 'max_width', 0);
		...
		if( $file['width'] ) {
			...
			if( $min_width && $file['width'] < $min_width ) {
				...
				// min width
				$errors['min_width'] = sprintf(__('Image width must be at least %dpx.', 'gcacf'), $min_width );
				
			} elseif( $max_width && $file['width'] > $max_width ) {
				
				// min width
				$errors['max_width'] = sprintf(__('Image width must not exceed %dpx.', 'gcacf'), $max_width );
		...
		// height
		$min_height = (int) gcacf_maybe_get($field, 'min_height', 0);
		$max_height = (int) gcacf_maybe_get($field, 'max_height', 0);
		...
		if( $file['height'] ) {
			...
			if( $min_height && $file['height'] < $min_height ) {
				...
				// min height
				$errors['min_height'] = sprintf(__('Image height must be at least %dpx.', 'gcacf'), $min_height );
				
			}  elseif( $max_height && $file['height'] > $max_height ) {
				
				// min height
				$errors['max_height'] = sprintf(__('Image height must not exceed %dpx.', 'gcacf'), $max_height );
	...
	if( $file['size'] ) {
		...
		$min_size = gcacf_maybe_get($field, 'min_size', 0);
		$max_size = gcacf_maybe_get($field, 'max_size', 0);
		
		if( $min_size && $file['size'] < gcacf_get_filesize($min_size) ) {
				
			// min width
			$errors['min_size'] = sprintf(__('File size must be at least %s.', 'gcacf'), gcacf_format_filesize($min_size) );
			
		} elseif( $max_size && $file['size'] > gcacf_get_filesize($max_size) ) {
				
			// min width
			$errors['max_size'] = sprintf(__('File size must must not exceed %s.', 'gcacf'), gcacf_format_filesize($max_size) );
	...
	if( $file['type'] ) {
		...
		$mime_types = gcacf_maybe_get($field, 'mime_types', '');
		...
		if( !empty($mime_types) && !in_array($file['type'], $mime_types) ) {
			...
			if( count($mime_types) > 1 ) {
				...
				$mime_types[] = $last2 . ' ' . __('or', 'gcacf') . ' ' . $last1;
				
			}
			
			$errors['mime_types'] = sprintf(__('File type must be %s.', 'gcacf'), implode(', ', $mime_types) );
	...
	*/
	$errors = apply_filters( "gcacf/validate_attachment/type={$field['type']}",	$errors, $file, $attachment, $field, $context );
	$errors = apply_filters( "gcacf/validate_attachment/name={$field['_name']}", 	$errors, $file, $attachment, $field, $context );
	$errors = apply_filters( "gcacf/validate_attachment/key={$field['key']}", 	$errors, $file, $attachment, $field, $context );
	$errors = apply_filters( "gcacf/validate_attachment", 						$errors, $file, $attachment, $field, $context );
...
add_filter('gcacf/settings/uploader', '_gcacf_settings_uploader');

function _gcacf_settings_uploader( $uploader ) {
...
/*
function gcacf_translate_keys( $array, $keys ) {
	...
	foreach( $keys as $k ) {
		...
		// translate
		$array[ $k ] = gcacf_translate( $array[ $k ] );
...
function gcacf_translate( $string ) {
	
	// vars
	$l10n = gcacf_get_setting('l10n');
	$textdomain = gcacf_get_setting('l10n_textdomain');
	...
	if( is_array($string) ) {
		...
		return array_map('gcacf_translate', $string);
	...
	if( gcacf_get_setting('l10n_var_export') ){
...
function gcacf_maybe_add_action( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
...
function gcacf_is_row_collapsed( $field_key = '', $row_index = 0 ) {
	
	// collapsed
	$collapsed = gcacf_get_user_setting('collapsed_' . $field_key, '');
	
	
	// cookie fallback ( version < 5.3.2 )
	if( $collapsed === '' ) {
		
		$collapsed = gcacf_extract_var($_COOKIE, "gcacf_collapsed_{$field_key}", '');
		$collapsed = str_replace('|', ',', $collapsed);
		
		
		// update
		gcacf_update_user_setting( 'collapsed_' . $field_key, $collapsed );
...
function gcacf_get_attachment_image( $attachment_id = 0, $size = 'thumbnail' ) {
...
function gcacf_get_post_thumbnail( $post = null, $size = 'thumbnail' ) {
	...
	// vars
	$thumb_id = $post->ID;
	$mime_type = gcacf_maybe_get(explode('/', $post->post_mime_type), 0);
	...
	$data['url'] = gcacf_maybe_get($data['url'], 0);
...
function gcacf_is_ajax( $action = '' ) {
	...
	// check $action
	if( $action && gcacf_maybe_get($_POST, 'action') !== $action ) {
...
function gcacf_format_date( $value, $format ) {
...
*  gcacf_dev_log
*
*  Used to log variables only if GCACF_DEV is defined
*
*  @date	25/8/18
*  @since	5.7.4
*
...
*  @return	void
*/
function gcacf_dev_log() {
	if( defined('GCACF_DEV') && GCACF_DEV ) {
		call_user_func_array('gcacf_log', func_get_args());
	}
}
...
*  gcacf_doing
*
*  This function will tell GCACF what task it is doing
*
*  @type	function
*  @date	28/06/2016
*  @since	5.3.8
...
function gcacf_doing( $event = '', $context = '' ) {
	
	gcacf_update_setting( 'doing', $event );
	gcacf_update_setting( 'doing_context', $context );
...
*  gcacf_is_doing
*
*  This function can be used to state what GCACF is doing, or to check
*
*  @type	function
*  @date	28/06/2016
*  @since	5.3.8
...
function gcacf_is_doing( $event = '', $context = '' ) {
	...
	// task
	if( gcacf_get_setting('doing') === $event ) {
	...
	// context
	if( $context && gcacf_get_setting('doing_context') !== $context ) {
...
/*
*  gcacf_is_plugin_active
*
*  This function will return true if the GCACF plugin is active
...
function gcacf_is_plugin_active() {
	
	// vars
	$basename = gcacf_get_setting('basename');
...
function gcacf_get_filters() {
	
	// get
	$filters = gcacf_raw_setting('filters');
...
function gcacf_update_filters( $filters ) {
	return gcacf_update_setting('filters', $filters);
}
...
function gcacf_enable_filter( $filter = '' ) {
	
	// get 
	$filters = gcacf_get_filters();
	
	// append
	$filters[ $filter ] = true;
	
	// update
	gcacf_update_filters( $filters );
}


/*
*  gcacf_disable_filter
*
...
function gcacf_disable_filter( $filter = '' ) {
...
/*
*  gcacf_enable_filters
*
*  GCACF uses filters to modify field group and field data
*  This function will enable them allowing GCACF to interact with all data
*
*  @type	function
*  @date	14/07/2016
*  @since	5.4.0
...
*
*  GCACF uses filters to modify field group and field data
*  This function will disable them allowing GCACF to interact only with raw DB data
*
*  @type	function
*  @date	14/07/2016
*  @since	5.4.0
...
function gcacf_disable_filters() {
...
function gcacf_is_filter_enabled( $filter = '' ) {
...
function gcacf_send_ajax_results( $response ) {
...
function gcacf_is_sequential_array( $array ) {
...
function gcacf_is_associative_array( $array ) {
...
function gcacf_add_array_key_prefix( $array, $prefix ) {
...
function gcacf_remove_array_key_prefix( $array, $prefix ) {
...
function gcacf_strip_protocol( $url ) {
...
/*
*  gcacf_connect_attachment_to_post
*
...
function gcacf_connect_attachment_to_post( $attachment_id = 0, $post_id = 0 ) {
...
function gcacf_get_post_templates() {
...
function gcacf_parse_markdown( $text = '' ) {
...
/**
*  gcacf_convert_rules_to_groups
*
*  Converts an array of rules from GCACF4 to an array of groups for GCACF5
*
*  @date	25/8/18
*  @since	5.7.4
*
*  @param	array $rules An array of rules.
*  @param	string $anyorall The anyorall setting used in GCACF4. Defaults to 'any'.
*  @return	array
*/
function gcacf_convert_rules_to_groups( $rules, $anyorall = 'any' ) {
	...
	foreach( $rules as $rule ) {
		...
		// extract vars
		$group = gcacf_extract_var( $rule, 'group_no' );
		$order = gcacf_extract_var( $rule, 'order_no' );
