(function($, undefined){
	...
	*/
	...
	var fieldGroupManager = new gcacf.Model({
		...
		onSubmit: function( e, $el ){
			...
			if( !$title.val() ) {
				...
				// alert
				alert( gcacf.__('Field group title is required') );
		...
		onClickTrash: function( e ){
			var result = confirm( gcacf.__('Move to trash. Are you sure?') );
			if( !result ) {
				e.preventDefault();
			}
		},
	...
	*/
	...
	var screenOptionsManager = new gcacf.Model({
		...
		initialize: function(){
			...
			var $append = $('#gcacf-append-show-on-screen');
		...
		onChange: function( e, $el ) {
			...
			gcacf.updateUserSetting('show_field_keys', val);
			this.render();
		},
		
		render: function(){
			if( this.isChecked() ) {
				$('#gcacf-field-group-fields').addClass('show-field-keys');
			} else {
				$('#gcacf-field-group-fields').removeClass('show-field-keys');
			}
		}
	...
	*/
	...
	var appendFieldManager = new gcacf.Model({
		...
		onNewField: function( field ){
			...
			// ul
			var $div = $sibling.children('.gcacf-input');
			var $ul = $div.children('ul');
			
			// create ul
			if( !$ul.length ) {
				$div.wrapInner('<ul class="gcacf-hl"><li></li></ul>');
				$ul = $div.children('ul');
			}
			
			// li
			var html = field.$('.gcacf-input').html();
			var $li = $('<li>' + html + '</li>');
			$ul.append( $li );
...
(function($, undefined){
	...
	gcacf.FieldObject = gcacf.Model.extend({
		
		// class used to avoid nested event triggers
		eventScope: '.gcacf-field-object',
		...
		$setting: function( name ){
			return this.$('.gcacf-field-settings:first > .gcacf-field-setting-' + name);
		},
		
		getParent: function(){
			return gcacf.getFieldObjects({ child: this.$el, limit: 1 }).pop();
		},
		
		getParents: function(){
			return gcacf.getFieldObjects({ child: this.$el });
		},
		
		getFields: function(){
			return gcacf.getFieldObjects({ parent: this.$el });
		},
		
		getInputName: function(){
			return 'gcacf_fields[' + this.get('id') + ']';
		},
		...
		getTypeLabel: function(){
			...
			var types = gcacf.get('fieldTypes');
		...
		render: function(){
			...
			// update menu order
			$handle.find('.gcacf-icon').html( parseInt(menu_order) + 1 );
			...
			gcacf.doAction('render_field_object', this);
		},
		
		refresh: function(){
			gcacf.doAction('refresh_field_object', this);
		},
		...
		open: function(){
			...
			// action (open)
			gcacf.doAction('open_field_object', this);
			this.trigger('openFieldObject');
			
			// action (show)
			gcacf.doAction('show', $settings);
		},
		...
		close: function(){
			...
			// action (close)
			gcacf.doAction('close_field_object', this);
			this.trigger('closeFieldObject');
			
			// action (hide)
			gcacf.doAction('hide', $settings);
		},
		
		serialize: function(){
			return gcacf.serialize( this.$el, this.getInputName() );
		},
		...
		save: function( type ){
			...
			// action
			gcacf.doAction('save_field_object', this, type);
		},
		...
		submit: function(){
			...
			// action
			gcacf.doAction('submit_field_object', this);
		},
		...
		onChange: function( e, $el ){
			...
			// action for 3rd party customization
			gcacf.doAction('change_field_object', this);
		},
		...
		onChanged: function( e, $el, name, value ){
			...
			// action for 3rd party customization
			gcacf.doAction('change_field_object_' + name, this, value);
		},
		...
		onChangeLabel: function( e, $el ){
			...
			// render name
			if( this.prop('name') == '' ) {
				var name = gcacf.applyFilters('generate_field_object_name', gcacf.strSanitize(label), this);
				this.prop('name', name);
			}
		},
		...
		onChangeName: function( e, $el){
			...
			if( name.substr(0, 6) === 'field_' ) {
				alert( gcacf.__('The string "field_" may not be used at the start of a field name') );
			}
		},
		...
		delete: function( args ){
			...
			if( id ) {
				var $input = $('#_gcacf_delete_fields');
				var newVal = $input.val() + '|' + id;
				$input.val( newVal );
			}
			
			// action
			gcacf.doAction('delete_field_object', this);
		...
		removeAnimate: function(){
			...
			var $fields = gcacf.findFieldObjects({
				sibling: this.$el
			});
			...
			gcacf.remove({
				...
				complete: function(){
					field.remove();
					gcacf.doAction('removed_field_object', field, $list);
				}
			});
			
			// action
			gcacf.doAction('remove_field_object', field, $list);
		},
		
		duplicate: function(){
			
			// vars
			var newKey = gcacf.uniqid('field_');
			
			// duplicate
			var $newField = gcacf.duplicate({
				target: this.$el,
				search: this.get('id'),
				replace: newKey,
			});
			...
			// get instance
			var newField = gcacf.getFieldObject( $newField );
			...
			// action
			gcacf.doAction('duplicate_field_object', this, newField);
			gcacf.doAction('append_field_object', newField);
		},
		...
		wipe: function(){
			...
			// action
			gcacf.doAction('wipe_field_object', this);
		},
		...
		move: function(){
			...
			if( changed ) {
				alert( gcacf.__('This field cannot be moved until its changes have been saved') );
				return;
			}
			...
			var step1 = function(){
				...
				popup = gcacf.newPopup({
					title: gcacf.__('Move Custom Field'),
					loading: true,
					width: '300px'
				});
				...
				// ajax
				var ajaxData = {
					action:		'gcacf/field_group/move_field',
					field_id:	id
				};
			...
			var step3 = function( e, $el ){
				...
				// disable
				gcacf.startButtonLoading( popup.$('.button') );
		...
		changeType: function( newType ){
			...
			var prevClass = gcacf.strSlugify( 'gcacf-field-object-' + prevType );
			var newClass = gcacf.strSlugify( 'gcacf-field-object-' + newType );
			...
			// load settings
			var $loading = $('<tr class="gcacf-field"><td class="gcacf-label"></td><td class="gcacf-input"><div class="gcacf-loading"></div></td></tr>');
			this.$setting('conditional_logic').before( $loading );
			
			// ajax
			var ajaxData = {
				action: 'gcacf/field_group/render_field_settings',
...
(function($, undefined){
	...
	*/
	...
	var modelId = function( type ) {
		return gcacf.strPascalCase( type || '' ) + 'FieldSetting';
	};
	...
	*/
	...
	gcacf.registerFieldSetting = function( model ){
	...
	*/
	...
	gcacf.newFieldSetting = function( field ){
		
		// vars
		var type = field.get('setting') || '';
		var name = field.get('name') || '';
		var mid = modelId( type + ' ' + name );
		var model = gcacf.models[ mid ] || null;
	...
	*/
	...
	gcacf.getFieldSetting = function( field ) {
		
		// allow jQuery
		if( field instanceof jQuery ) {
			field = gcacf.getField(field);
		}
	...
	*/
	...
	var settingsManager = new gcacf.Model({
		actions: {
			'new_field': 'onNewField'
		},
		onNewField: function( field ){
			field.setting = gcacf.newFieldSetting( field );
		}
	});
	...
	*/
	...
	gcacf.FieldSetting = gcacf.Model.extend({
		...
		setup: function( field ){
			...
			this.$el = $field;
			this.field = field;
			this.$fieldObject = $field.closest('.gcacf-field-object');
			this.fieldObject = gcacf.getFieldObject( this.$fieldObject );
	...
	*/
	...
	var DisplayFormatFieldSetting = gcacf.FieldSetting.extend({
		type: '',
		name: '',
		render: function(){
	...
	gcacf.registerFieldSetting( DatePickerDisplayFormatFieldSetting );
	gcacf.registerFieldSetting( DatePickerReturnFormatFieldSetting );
	...
	*/
	...
	gcacf.registerFieldSetting( DateTimePickerDisplayFormatFieldSetting );
	gcacf.registerFieldSetting( DateTimePickerReturnFormatFieldSetting );
	...
	*/
	...
	gcacf.registerFieldSetting( TimePickerDisplayFormatFieldSetting );
	gcacf.registerFieldSetting( TimePickerReturnFormatFieldSetting );
...
(function($, undefined){
	...
	*/
	...
	var ConditionalLogicFieldSetting = gcacf.FieldSetting.extend({
		type: '',
		...
		renderField: function(){
			...
			// loop
			gcacf.getFieldObjects().map(function( fieldObject ){
				...
				if( fieldObject.cid === cid  ) {
					choice.text += gcacf.__('(this field)');
			...
			if( !choices.length ) {
				choices.push({
					id: '',
					text: gcacf.__('No toggle fields available'),
				});
			}
			
			// render
			gcacf.renderSelect( $select, choices );
		...
		renderOperator: function(){
			...
			// get selected field
			var $field = gcacf.findFieldObject( this.ruleData('field') );
			var field = gcacf.getFieldObject( $field );
			...
			conditionTypes.map(function( model ){
				choices.push({
					...
					text:	gcacf.strEscape(model.prototype.label)
				});
			});
		...
		renderValue: function(){
			...
			if( choices instanceof Array ) {
				...
				gcacf.renderSelect( $newSelect, choices );
		...
		addGroup: function(){
			...
			// duplicate
			var $group2 = gcacf.duplicate( $group );
			
			// update h4
			$group2.find('h4').text( gcacf.__('or') );
		...
		onClickAdd: function( e, $el ){
			...
			// duplciate
			var $rule = gcacf.duplicate( $el.closest('.rule') );
	...
	gcacf.registerFieldSetting( ConditionalLogicFieldSetting );
	...
	*/
	...
	var conditionalLogicHelper = new gcacf.Model({
		actions: {
...
(function($, undefined){
	...
	*/
	...
	gcacf.findFieldObject = function( key ){
		return gcacf.findFieldObjects({
			key: key,
			limit: 1
		});
	};
	...
	*/
	...
	gcacf.findFieldObjects = function( args ){
		
		// vars
		args = args || {};
		var selector = '.gcacf-field-object';
		var $fields = false;
	...
	*/
	...
	gcacf.getFieldObject = function( $field ){
		
		// allow key
		if( typeof $field === 'string' ) {
			$field = gcacf.findFieldObject( $field );
		}
		
		// instantiate
		var field = $field.data('gcacf');
		if( !field ) {
			field = gcacf.newFieldObject( $field );
		}
	...
	*/
	...
	gcacf.getFieldObjects = function( args ){
		
		// query
		var $fields = gcacf.findFieldObjects( args );
		
		// loop
		var fields = [];
		$fields.each(function(){
			var field = gcacf.getFieldObject( $(this) );
			fields.push( field );
		});
	...
	*/
	...
	gcacf.newFieldObject = function( $field ){
		
		// instantiate
		var field = new gcacf.FieldObject( $field );
		
		// action
		gcacf.doAction('new_field_object', field);
	...
	*/
	...
	var eventManager = new gcacf.Model({
		...
		addFieldActions: function( action ){
			...
			var callback = function( $el /*, arg1, arg2, etc*/ ){
				...
				// vars
				var fieldObjects = gcacf.getFieldObjects({ parent: $el });
			...
			// add actions
			gcacf.addAction(action, callback, 5);
			gcacf.addAction(pluralAction, pluralCallback, 5);
			gcacf.addAction(singleAction, singleCallback, 5);
	...
	*/
	...
	var fieldManager = new gcacf.Model({
		
		id: 'fieldManager',
		
		events: {
			'submit #post':					'onSubmit',
			'mouseenter .gcacf-field-list': 	'onHoverSortable',
		...
		onSubmit: function( e, $el ){
			...
			// vars
			var fields = gcacf.getFieldObjects();
		...
		onHoverSortable: function( e, $el ){
			...
			// sortable
			$el.sortable({
				handle: '.gcacf-sortable-handle',
				connectWith: '.gcacf-field-list',
				start: function( e, ui ){
					var field = gcacf.getFieldObject( ui.item );
			        ui.placeholder.height( ui.item.height() );
			        gcacf.doAction('sortstart_field_object', field, $el);
			    },
				update: function( e, ui ){
					var field = gcacf.getFieldObject( ui.item );
					gcacf.doAction('sortstop_field_object', field, $el);
				}
			});
		},
		...
		onDuplicateField: function( field, newField ){
			...
			if( children.length ) {
				...
				// action
				gcacf.doAction('duplicate_field_objects', children, newField, field);
			}
		...
		onClickAdd: function( e, $el ){
			var $list = $el.closest('.gcacf-tfoot').siblings('.gcacf-field-list');
			this.addField( $list );
		},
		...
		addField: function( $list ){
			...
			// vars
			var html = $('#tmpl-gcacf-field').html();
			var $el = $(html);
			...
			// action
			gcacf.doAction('add_field_object', newField);
			gcacf.doAction('append_field_object', newField);
		}
	});
...
(function($, undefined){
	...
	*/
	...
	var locationManager = new gcacf.Model({
		...
		initialize: function(){
			this.$el = $('#gcacf-field-group-locations');
		},
		...
		changeRule: function( $rule ){
			...
			// ajaxdata
			var ajaxdata = {};
			ajaxdata.action = 'gcacf/field_group/render_location_rule';
			ajaxdata.rule = gcacf.serialize( $rule, prefix );
			ajaxdata.rule.id = $rule.data('id');
			ajaxdata.rule.group = $group.data('id');
			
			// temp disable
			gcacf.disable( $rule.find('td.value') );
...
(function($, undefined){
	...
	var _gcacf = gcacf.getCompatibility( gcacf );
	
	/**
	*  fieldGroupCompatibility
	*
	*  Compatibility layer for extinct gcacf.field_group 
	*
	*  @date	15/12/17
	*  @since	5.7.0
	*
	...
	*/
	...
	_gcacf.field_group = {
		...
		save_field: function( $field, type ){
			...
			gcacf.getFieldObject( $field ).save( type );
		},
		
		delete_field: function( $field, animate ){
			animate = (animate !== undefined) ? animate : true;
			gcacf.getFieldObject( $field ).delete({
				animate: animate
			});
		},
		
		update_field_meta: function( $field, name, value ){
			gcacf.getFieldObject( $field ).prop( name, value );
		},
		
		delete_field_meta: function( $field, name ){
			gcacf.getFieldObject( $field ).prop( name, null );
		}
	};
	...
	*
	*  Compatibility layer for extinct gcacf.field_group.field_object
	*
	*  @date	15/12/17
	*  @since	5.7.0
	*
	...
	*/
	...
	_gcacf.field_group.field_object = gcacf.model.extend({
		...
		selector: function(){
			...
			if( type ) {
				selector += '-' + type;
				selector = gcacf.str_replace('_', '-', selector);
			}
		...
		_add_action: function( name, callback ) {
			...
			// add action
			gcacf.add_action( this.tag(name), function( $field ){
		...
		_add_filter: function( name, callback ) {
			...
			// add action
			gcacf.add_filter( this.tag(name), function( $field ){
		...
		setting: function( name ) {
			...
			return this.$settings.find('> .gcacf-field-setting-' + name);
	...
	*/
	...
	var actionManager = new gcacf.Model({
		...
		onOpenFieldObject: function( field ){
			gcacf.doAction('open_field', field.$el);
			gcacf.doAction('open_field/type=' + field.get('type'), field.$el);
			
			gcacf.doAction('render_field_settings', field.$el);
			gcacf.doAction('render_field_settings/type=' + field.get('type'), field.$el);
		},
		
		onCloseFieldObject: function( field ){
			gcacf.doAction('close_field', field.$el);
			gcacf.doAction('close_field/type=' + field.get('type'), field.$el);
		},
		
		onAddFieldObject: function( field ){
			gcacf.doAction('add_field', field.$el);
			gcacf.doAction('add_field/type=' + field.get('type'), field.$el);
		},
		
		onDuplicateFieldObject: function( field ){
			gcacf.doAction('duplicate_field', field.$el);
			gcacf.doAction('duplicate_field/type=' + field.get('type'), field.$el);
		},
		
		onDeleteFieldObject: function( field ){
			gcacf.doAction('delete_field', field.$el);
			gcacf.doAction('delete_field/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectType: function( field ){
			gcacf.doAction('change_field_type', field.$el);
			gcacf.doAction('change_field_type/type=' + field.get('type'), field.$el);
		...
		onChangeFieldObjectLabel: function( field ){
			gcacf.doAction('change_field_label', field.$el);
			gcacf.doAction('change_field_label/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectName: function( field ){
			gcacf.doAction('change_field_name', field.$el);
			gcacf.doAction('change_field_name/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectParent: function( field ){
			gcacf.doAction('update_field_parent', field.$el);
		}
	});
